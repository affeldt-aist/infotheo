...
Definition idealized_enc (p : partyT) (m r : msgT) : idealized_enc_party := (p, m).

Definition idealized_key (p : partyT) (k : key_type) (s : msgT) : idealized_pkey := 
  (p, k, s).

Definition idealized_dec (dk : idealized_pkey) (e : idealized_enc_party) : option msgT :=
  match dk, e with
...
Definition idealized_Emul (e1 e2 : idealized_enc_party) : idealized_enc_party := 
  let (p1, m1) := e1 in
  let (_, m2) := e2 in
  (p1, m1 + m2).

(* Homomorphic scalar multiplication: multiplies message by scalar *)
Definition idealized_Epow (e : idealized_enc_party) (k : msgT) : idealized_enc_party :=
  match e with
  | (p, m) => (p, m * k)
  end.
...
Local Notation "x +mr y" := (msg_rand_add IT x y) (at level 50, left associativity).
...
Lemma idealized_Emul_addM : forall (p : partyT),
  {morph E[ p ] : x y / x +mr y >-> idealized_Emul x y}.
Proof.
  move=> p [m1 r1] [m2 r2].
...
Lemma idealized_Epow_mulM : forall (p : partyT) (k : msgT),
  {morph E[ p ] : mr / (mr.1 * k, idealized_rand_pow mr.2 k) >-> idealized_Epow mr k}.
Proof.
  move=> p k [m r].
