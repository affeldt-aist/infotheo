...
(*   Fazio, R., Nicolosi, A. [2002]. "Cryptographic accumulators: Definitions,*)
(*   Constructions and Applications"                                          *)
(*   Fousse, L., Lafourcade, P., & Alnuaimi, M. [2011]. "Benaloh's            *)
(*   Dense Probabilistic Encryption Revisited"                                *)
(*                                                                            *)
(******************************************************************************)
...
(* Private key is alpha = phi(n)/r, which requires knowing phi(n).
   In practice, phi(n) = (p-1)(q-1) is computable only with the prime factors. *)

(* Decryption: search for m such that y^(alpha*m) = c^alpha.
   This is a discrete log in the subgroup generated by y^alpha.
   Since r is small (block size), exhaustive search is feasible. *)
Definition benaloh_dec (priv_alpha : nat) (c : 'Z_n) : option 'Z_r :=
  [pick m : 'Z_r | (val y) ^+ (priv_alpha * m) == c ^+ priv_alpha].
...
Lemma benaloh_dec_correct (m : 'Z_r) (u : {unit 'Z_n}) :
  benaloh_dec alpha (benaloh_enc m u) = Some m.
Proof.
  rewrite /benaloh_dec.
  case: pickP => [m' /eqP Hm' | Hcontra].
  - (* Found some m', show it equals m *)
    congr Some.
    rewrite ciphertext_to_alpha in Hm'.
    exact: y_alpha_injective Hm'.
  - (* Contradiction: no m found, but m itself should work *)
    exfalso.
    move: (Hcontra m).
    by rewrite ciphertext_to_alpha eqxx.
Qed.
