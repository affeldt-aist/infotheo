...
Require Import realType_ext ssr_ext ssralg_ext bigop_ext realType_logb.
...
case/boolP : (type.d P a == 0) => [/eqP|] Pa0.
  ...
  by rewrite H expr0 !(mulr0,mul0r) exp2_0.
...
case/boolP : (V a b == 0) => [/eqP|] Vab0.
  suff -> : N( a, b | [seq x ``_ i | i <- enum 'I_n], [seq y ``_ i | i <- enum 'I_n]) = O.
    by rewrite expr0 Vab0 !(mulr0,mul0r,addr0,add0r,oppr0,exp2_0).
  move: Hy; rewrite in_set => /forallP/(_ a)/forallP/(_ b)/eqP => ->.
  by rewrite jtype_0_jtypef.
...
Definition exp_cdiv :=
  if (type.d P) |- V <<b W
  then Exp 2 (- n%:R * D(V || W | P))
  else 0.

Lemma exp_cdiv_left (H : P |- V << W) : exp_cdiv = Exp 2 (- n%:R * D(V || W | P)).
Proof.
rewrite /exp_cdiv.
...
Lemma dmc_exp_cdiv_cond_entropy :
  W ``(y | x) = exp_cdiv P V W * Exp 2 (- n%:R * `H(V | P)).
Proof.
rewrite /exp_cdiv.
case : ifP => Hcase.
