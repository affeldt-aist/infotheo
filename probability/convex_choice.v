...
Module AltToBeaulieu(A : AltConvSpace) <: BeaulieuSpace.
Module B := NToBin(A).
Import A B.
Definition T := A.T.
...
Module BeaulieuToAlt(B : BeaulieuSpace) <: AltConvSpace.
Import B.
Definition T := B.T.
...
have Hmap' i : FDistMap.d h' d i = (\sum_j d (h i) * e (h i) j)%R.
  ...
  rewrite (bigD1 (h i)) /=; last by rewrite /h /h' enum_valK_in eqxx.
  ...
  case: (f a) => j /= /orP[/forallP /= |] Ha.
    have Ha0 k : (d k * e k a = 0)%R.
      case/boolP: (k \in fdist_supp d) => Hk.
        move: (Ha (h' k)).
        rewrite inE negbK /h/h' enum_rankK_in // => /eqP ->; by rewrite mulR0.
      move: Hk; rewrite inE negbK => /eqP -> ; by rewrite mul0R.
    rewrite (proj2 (prsumr_eq0P _)) ?(if_same,Ha0,mulR0) //.
    move=> k _; by apply mulR_ge0.
  case: eqP => Hi.
    subst i.
    rewrite (bigD1 (h j)) //= big1 ?addR0 // => *; by rewrite (neqj j).
  rewrite (neqj j) //; move/eqP: Hi; by apply contra => /eqP/enum_val_inj ->.
congr aConvn.
  apply fdist_ext => /= i; by rewrite Hmap.
apply funext => i /=.
have HF : FDistMap.d h' d i != 0%R.
  rewrite fdistE /=.
  apply/eqP => /prsumr_eq0P => H.
  have: h i \in fdist_supp d by apply enum_valP.
  by rewrite inE H ?eqxx // /h /h' enum_valK_in.
rewrite (@cnidem (<a>_(e (h i)) g)); last first.
  move=> /= j; rewrite inE.
...
rewrite FDistPart.dE; last first.
  ...
  rewrite (eq_bigr (e (h i))).
    ...
    apply paddR_neq0 => //.
      ...
      apply/rsumr_ge0 => *.
      by apply/mulR_ge0.
    left; move: (enum_valP i); by rewrite inE.
  move=> /= k _.
  case: ifP => //.
  case: (f k) => /= x /orP[].
    move/forallP/(_ i).
...
case: eqP ji => ik.
  subst k; by rewrite jk.
rewrite inE negbK => /eqP ->.
by rewrite mulR0 div0R.
Qed.
...
case/boolP: (j == p.2) => [/eqP -> | Hj]; last first.
  ...
  - move=> i; apply/negbTE; move: Hj; apply contra.
    rewrite -(enum_valK k) (can_eq enum_rankK).
...
Lemma avgE p (x y : R) : x <| p |> y = avg p x y. Proof. by []. Qed.
Lemma avg_oppD p x y : (- x <| p |> - y = - (x <| p |> y))%R.
Proof. by rewrite avgE /avg 2!mulRN -oppRD. Qed.
Lemma avg_mulDr p : right_distributive Rmult (fun x y => x <| p |> y).
Proof. by move=> x ? ?; rewrite avgE /avg mulRDr mulRCA (mulRCA x). Qed.
Lemma avg_mulDl p : left_distributive Rmult (fun x y => x <| p |> y).
Proof. by move=> x ? ?; rewrite avgE /avg mulRDl -mulRA -(mulRA p.~). Qed.
...
Definition convex_function_at_Convn f n (a : 'I_n -> A) (d : {fdist 'I_n}) :=
  f (<|>_d a) <= <|>_d (f \o a).
...
rewrite /Leconv /probinvn /= 3!avgE /=.
...
Proof. by rewrite /convex_function_at /Leconv /= avg_oppD leR_oppl oppRK. Qed.
...
by rewrite /Leconv /= avg_oppD leR_oppl oppRK.
Qed.
...
rewrite avgE 2!mulRBr addRAC addRA.
move: (H1 p q t) => {}H1.
...
rewrite conv_leoppD leoppP avgE.
...
rewrite [in X in Leconv _ X]avgE /= onemK addRC.
rewrite /convex_function_at !avgE in H.
rewrite avgE /= onemK addRC.
