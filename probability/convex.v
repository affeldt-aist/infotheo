...
Lemma onem_probR_gt0 (p: {prob R}) : (0 <= p.~)%coqR.
Admitted.
...
Let avgE p a b : avg p a b = (p * a + p.~ * b)%coqR.
Proof. by []. Qed.
...
Lemma avgRE p (x y : R) : x <| p |> y = (p * x + p.~ * y)%coqR. Proof. by []. Qed.
