...
case/boolP : (p == 1%:pr) => p1.
  ...
  apply val_inj => /=.
  by rewrite fdist_delE fdistD1E (eq_sym (lift _ _)) (negbTE (neq_lift _ _)) -Hp.
  exact/val_inj.
  exact/val_inj.
rewrite convnE; congr (_ <| _ |> _).
rewrite convn2E /fdist_del_idx ltnn /=; congr (g _ <| _ |> g _).
...
move: (FDist.ge0 d ord0); rewrite leR_eqVlt => -[/esym d00|d00].
  ...
  have H2 : PermFDist.d d (tperm ord0 (Ordinal (erefl (1 < 2)))) = fdist1 ord0.
    apply/fdist_ext => /= i; rewrite PermFDist.dE fdist1E.
...
transitivity (g ord0 <| probfdist d ord0 |> (<|>_(PermFDist.d (fdist_del dmax1) s') ((fun x => g (fdist_del_idx ord0 x)) \o s'))).
  by rewrite -IH.
transitivity (g (s ord0) <| probfdist d ord0 |> (<|>_(PermFDist.d (fdist_del dmax1) s') ((fun x => g (fdist_del_idx ord0 x)) \o s'))).
  by rewrite H.
...
congr (Convn _ _).
  apply/fdist_ext => /= j.
  rewrite !PermFDist.dE !fdist_delE !fdistD1E !PermFDist.dE.
  rewrite /s' /=.
  rewrite permE.
  rewrite /f /=.
...
case/boolP : (d (lift ord0 ord0) == 1 - d ord0 :> R)%R => K.
  case/boolP : (d (lift ord0 ord0) == 1%R :> R) => [|d11].
    by rewrite fdist1E1 => /eqP ->; rewrite PermFDist.d1 !ConvnFDist1 /= permKV.
  rewrite convnE.
  rewrite [in RHS]convnE.
  ...
  rewrite (_ : <|>_ _ _ = g ord0); last first.
    have /eqP : (fdist_del K') ord0 = 1%R.
      rewrite fdist_delE fdistD1E /= !PermFDist.dE H !permE /=.
...
congr (_ <| _ |> _).
  apply val_inj => /=.
  by rewrite !fdist_delE !fdistD1E /= !PermFDist.dE H !permE.
...
congr (Convn _ _).
  apply/fdist_ext => j.
  rewrite !(fdist_delE,PermFDist.dE,fdistD1E).
  rewrite H !permE /=.
  field.
  split.
