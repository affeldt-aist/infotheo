...
(*                      distributions                                         *)
(*     ConvnFSDist.d == of type {fdist 'I_n} -> ('I_n -> {dist A}) -> {dist A}*)
(*                      convex combination of n distributions                 *)
(*      ConvFSDist.d == of type prob -> {dist A} -> {dist A} -> {dist A}      *)
(*                      convex combination of two distributions               *)
(*                                                                            *)
(* Free convex spaces in terms of finitely-supported distributions:           *)
(*  {dist A} and @ConvFSDist.d A are used to form an instance of convType,    *)
(*  this shows that finitely-supported distributions over a choiceType form a *)
(*  convex space                                                              *)
(*   FSDistfmap f is shown to have the structure of an affine function        *)
...
Lemma FSDistBindE (B : choiceType) (a b : {dist A}) (f : A -> {dist B}) (p : prob)
  (a0 : A) (b0 : B) (b0a0 : b0 \in finsupp (f a0)) :
  p != 0%:pr ->
  (FSDistBind.d a f) b0 = \sum_(i <- finsupp (a <|p|> b)) (a i * (f i) b0).
Proof.
move=> p0.
rewrite FSDistBind.dE.
...
case: ifPn => [/bigfcupP[dB] | Hb0].
  ...
  under eq_bigr.
    by move=> a1 _; rewrite dE (@mulRDl _ _ (f a1 b0)) -!mulRA; over.
  rewrite big_split /= -2!big_distrr /=; congr (_ * _ + _ * _)%R.
    exact/esym/(FSDistBindE _ _ b0a0).
  rewrite (@FSDistBindE _ b a _ (p.~)%:pr a0)// 1?convC//.
  apply: contra p1 => /eqP/(congr1 val) /= /onem_eq0 p1.
  exact/eqP/val_inj.
apply/esym/paddR_eq0; [exact/mulR_ge0 | exact/mulR_ge0 |].
suff: forall c, finsupp c `<=` finsupp (a <|p|> b) -> FSDistBind.d c f b0 = 0.
...
Lemma FSDistfmap_affine (A B : choiceType) (f : A -> B) : affine (FSDistfmap f).
...
rewrite -(@ssum_widen_finsupp x); last exact/ConvFSDist.finsupp_conv_subr.
by rewrite -(@ssum_widen_finsupp y)//; exact/ConvFSDist.finsupp_conv_subl.
Qed.
...
rewrite (eq_bigr (fun=> ScaledConvex.S1 x)); last first.
  move=> i _; rewrite fdist_of_fsE FSDist1.dE /= -(FSDist1.supp x).
...
Lemma Convn_of_FSDist_FSDistfmap (C D : convType) (f : {affine C -> D})
    (d : {dist C}) :
