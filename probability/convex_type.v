...
Lemma ConvDist_Add (A : finType) (g : 'I_n -> fdist A) (h : 'I_m -> fdist A) :
  ConvFDist.d
    (AddFDist.d d1 d2 p)
...
Lemma f1 (x : set A) (gX : g @` setT `<=` x `|` y)
  ...
  (\sum_(i < n) f i = 1)%R.
Proof.
rewrite /f -(FDist.pmf1 e) /=; apply eq_bigr => i _; rewrite ffunE.
...
Definition fdist_convMixin :=
  @ConvexSpace.Class _ (@Conv2FDist.d A)
  (@Conv2FDist.d1 A)
...
Import ScaledConvex.
Lemma convn_convdist (n : nat) (g : 'I_n -> fdist A) (d : {fdist 'I_n}) :
  \Conv_d g = ConvFDist.d d g.
Proof.
apply fdist_ext=> a; rewrite -[LHS]Scaled1RK.
rewrite (@S1_convn_proj _ _ (@^~ a \o @pos_ff _ \o @FDist.pmf _)); last first.
  move=> p x y /=; by rewrite /Conv /= Conv2FDist.dE.
rewrite big_scaleR ConvFDist.dE; apply eq_bigr => i _.
...
Lemma Conv2FDistdE (A : finType) (a b : fdist A) (p : prob) (x : A) :
  (a <| p |> b) x = a x <| p |> b x.
Proof. by rewrite Conv2FDist.dE. Qed.
...
Proof.
apply/fdist_ext => b0.
rewrite !(Conv2FDist.dE,FDistBind.dE) !big_distrr -big_split; apply eq_bigr => a0 _ /=.
by rewrite Conv2FDist.dE mulRDl 2!mulRA.
Qed.
