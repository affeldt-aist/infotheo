...
+ by move=> a p; rewrite -scalept_addR //; rewrite onemKC scalept1.
...
Proof. by apply/asboolP => x y p. Qed.
...
Proof. by apply/asboolP => ? ? ? _ _. Qed.
...
congr (CSet.mk _); exact/Prop_irrelevance.
Qed.
End CSet_interface.
...
Canonical hull_is_convex_set (Z : set A) : convex_set A :=
  CSet.mk (hull_is_convex Z).

Definition scaled_set (D : set A) :=
  [set x | if x is p *: a then D a else True].
...
(* NB(saikawa): hullI was removed from convex_choice.v as it was judged
   useless.  However, I want to keep it here since it exhibits a fundamental
...
exists sa1; split.
  ...
  + by move=> i Hi; apply scalept_scaled_set.
exists sa2; split.
...
Lemma avgA p q d0 d1 d2 :
  avg p d0 (avg q d1 d2) = avg [s_of p, q] (avg [r_of p, q]d0 d1) d2.
Proof.
by case d0; case d1; case d2 => d2' d1' d0'; rewrite /avg /unbox /= convA.
Qed.
...
Definition concave_function_at (f : A -> B) a b t :=
  convex_function_at (fun a => \opp{f a}) a b t.
...
Definition fdist_convMixin :=
  @ConvexSpace.Class _ (@ConvFDist.d A)
  (@ConvFDist.d1 A)
...
Import ScaledConvex.
Lemma convn_convndist (n : nat) (g : 'I_n -> fdist A) (d : {fdist 'I_n}) :
  \Conv_d g = ConvnFDist.d d g.
Proof.
apply fdist_ext=> a; rewrite -[LHS]Scaled1RK.
rewrite (@S1_convn_proj _ _ (@^~ a \o @pos_ff _ \o @FDist.f _)); last first.
  by move=> p x y /=; rewrite ConvFDist.dE.
rewrite big_scaleR ConvnFDist.dE; apply eq_bigr => i _.
by rewrite scaleR_scalept // Scaled1RK.
Qed.
End fdist_convex_space.

(* TODO *)
Section fdist_ordered_convex_space.
Variable A : finType.
Definition fdist_orderedConvMixin := (@OrderedConvexSpace.Mixin (fdist_convType A)).
End fdist_ordered_convex_space.

(*Lemma ConvFDistdE (A : finType) (a b : fdist A) (p : prob) (x : A) :
  (a <| p |> b) x = a x <| p |> b x.
Proof. by rewrite ConvFDist.dE. Qed.*)

Lemma FDistBindConv (A : finType) (B : finType)(p : prob) (dx dy : fdist A) (f : A -> fdist B) :
  FDistBind.d dx f <|p|> FDistBind.d dy f = FDistBind.d (dx <|p|> dy) f.
Proof.
apply/fdist_ext => b0.
rewrite !(ConvFDist.dE,FDistBind.dE) !big_distrr -big_split; apply eq_bigr => a0 _ /=.
by rewrite ConvFDist.dE mulRDl 2!mulRA.
Qed.

Lemma rsum_Conv (A : finType) (p : prob) (dx dy : fdist A):
...
Definition FSDist_convMixin :=
  @ConvexSpace.Class (FSDist.t A) (@ConvFSDist.d A)
  (@ConvFSDist.conv1 A)
  (@ConvFSDist.convmm A)
  (@ConvFSDist.convC A)
  (@ConvFSDist.convA' A).
...
Proof.
apply/asboolP=> x y t Hx Hy; apply addR_ge0; exact/mulR_ge0.
Qed.
