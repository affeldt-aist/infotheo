...
apply (scp.cpr_centropy (Y2:=X)(Y3:=E)) => c a b.
...
(* The 3 keys are defined in the previous section and they only
   need the `msg` type extracted from them to be fulfilled.
*)
...
(* E_charlie_v3 means it is encrypted (so generated) by the key of charlie.
   ...
   TODO: cannot use smc_inde.v here because RV2,
   RV msg and RV enc are all different types.
   They cannot be contained by one fset.
*)
...
(* So if a set of events is sized one (from an intersection, for example);
   ...
   in A :&: B. In the DSDP case, it means all events that `v2`
   encodes them as `v`, while `dotp2_rv us vs` also encodes them as `v`.
   If we unfold it more, the probability of such an event must be related to
   the probability that another random variable `us` outputs `(1, 0)`.
   But how possible it is, depends on the distribution of `us`:
   if an active adversary controls Alice, force `us` always output `(1, 0)`,
   then `v2 _|_ dotp2_rv us vs` is impossible.

   In contrast, if Alice is an fair player, the probability that `us`
   outputs that specific value is 1/m^2.
   Finally, if Bob enforce ZPK check to abort the protocol when that value is
   generated, `v2 _|_ dotp2_rv us vs` is a sure thing, and the protocol
...
(* It is saying that if Alice intentionally choses an const RV `us` that always
   outputs an pair of encoded values (1, 0), v2 will be disclosed.
*)
Lemma ext_v2_discloser:
  exists a b: msg,
    dotp2_rv [% (fun _ => a):{RV P -> msg},
    (fun _ => b):{RV P -> msg}] vs = v2.  
Proof.
rewrite /dotp2_rv.
exists 1.
exists 0.
...
Hypothesis s_aiv_neg0 : forall x a,
  `Pr[ [% s, alice_input_view] = (x, a) ] != 0.

Lemma neg_r_v2aivdotp_indep (v x : msg) (a : alice_input_view_valT):
  ~ P |= r _|_ [% v2, alice_input_view, dotp2_rv us vs].
Proof.
pose h := (fun o : (msg * alice_input_view_valT * msg) =>
  let '(_, aiv, _) := o in (g aiv):msg).
move/(inde_RV_comp idfun h).
