...
(* TODO: the `pvu1_unif` hypothesis cannot be held since vu1 := v1 \* u1.
   ...
   Maybe we can propose a variant to the DSDP protocol: by adding a new
   random nounce generated by parties other than Alice to the dot product
   result Alice cannot know the result of the dot product result.
   Then only an external examiner with both the result from Alice and the
   random nounce owner, the examiner can reveal the final result -- similar to
   the SMC-SPP protocol.
*)
