...
Let f (a : alice_input_view_valT) := let '(_, _, _, u2, u3, _, _) := a in
  (u2, u3).

Let g (a : alice_input_view_valT) := let '(_, v1, u1, _, _, _, _) := a in
  v1 * u1.

Lemma s_alt :
  s = dotp2_rv us vs \+ r.
Proof.
rewrite /s /us /vs /d3 /vu3r /d2 /vu3 /vu2 /r /dotp2_rv /dotp2 //= /add_RV.
