...
(*******************************************************************************)
(** ** Wait-for acyclicity via ranking functions                                *)
(*******************************************************************************)

Section ranking_acyclic.
Variable data : Type.

Lemma path_all_rank_lt (T : finType) (rel : T -> T -> bool)
    (rank : T -> nat) (x : T) (s : seq T) :
  (forall i j : T, rel i j -> (rank j < rank i)%N) ->
  path rel x s ->
  forall z, z \in s -> (rank z < rank x)%N.
Proof.
move=> Hrank; elim: s x => [|y s IHs] x //=.
move/andP => [Hxy Hpath] z; rewrite inE.
move/orP => [/eqP ->|Hz].
  exact: Hrank _ _ Hxy.
exact: ltn_trans (IHs _ Hpath _ Hz) (Hrank _ _ Hxy).
Qed.

Lemma wait_for_ranking_acyclic n (tps : n.-tuple (proc data)) (rank : 'I_n -> nat) :
  (forall i j : 'I_n, wait_for tps i j -> (rank j < rank i)%N) ->
  wait_for_acyclic tps.
Proof.
move=> Hrank s x Hpath.
elim: s x Hpath => [|y s IHs] x //.
rewrite [path _ _ _]/= => /andP [Hxy Hpath].
have Huniq := IHs y Hpath.
rewrite cons_uniq; apply/andP; split => //.
rewrite inE negb_or; apply/andP; split.
  apply/eqP => Heq; subst y.
  by move: (Hrank _ _ Hxy); rewrite ltnn.
apply/negP => Hxs.
have Hlt := @path_all_rank_lt _ (wait_for tps) rank y s Hrank Hpath x Hxs.
have Hlt2 := Hrank _ _ Hxy.
by move: (ltn_trans Hlt Hlt2); rewrite ltnn.
Qed.
...
(* Last relay concrete continuation: decrypt + re-encrypt for Alice *)
Lemma relay_last_recv_concrete :
  exists sv f_dec,
    relay_body (@ord_max n_relay) = Send 0 sv (Recv n_relay f_dec) /\
    forall m rr, f_dec (e_local (enc (ek (nat_to_party_id n_relay.+1)) m rr)) =
      Send 0 (e_local (enc (ek alice_idx) m (r2_relay (inord n_relay)))) Finish.
Proof.
rewrite /relay_body /=.
have -> : (n_relay == 0) = false by rewrite eqn0Ngt Hn_relay.
rewrite eqxx /pRecvDec_local /std_Recv_dec /Recv_param.
do 2 eexists; split; first by [].
move=> m rr; rewrite /comp std_from_enc_e_local /=.
rewrite (key_relay (@ord_max n_relay)) dec_correct /=.
have -> : @ord_max n_relay = inord n_relay :> 'I_n_relay.+1
...
Lemma alice_recv_to_send_foldr (j : nat) (Hj : (j < n_relay.+1)%N)
    (f : data -> proc data) (v : data) (c : cipher AHE) :
  @std_from_enc AHE v = Some c ->
  foldr (fun (fi : 'I_n_relay.+1 * nat) (cont : proc data) =>
           @alice_erase_body AHE ek n_relay u r rand_a fi.1 fi.2 cont)
        (@alice_erase_tail AHE n_relay dk v0 u r)
        (drop j (zip relays (iota 0 (size relays)))) =
  Recv (Ordinal Hj).+1 f ->
  let sv := e_local (@Emul AHE (@Epow AHE c (u (lift ord0 (Ordinal Hj))))
              (enc (ek (nat_to_party_id (Ordinal Hj).+1))
                   (r (Ordinal Hj)) (rand_a (Ordinal Hj)))) in
...
rewrite Hdrop /= /alice_erase_body (Hrelays_id (Ordinal Hj)) /= in Hfoldr.
rewrite /pRecvEnc_local /std_Recv_enc /Recv_param in Hfoldr.
case: Hfoldr => Hf; subst f; rewrite /comp Henc /=.
by rewrite /enc_pub_key.
Qed.
...
(* T2b: Like T2 but with concrete continuation for the Emul_local form *)
Lemma relay_inter_recv_concrete (j : 'I_n_relay.+1) (alice_c : cipher AHE)
    (sv0 : data) (f0 : data -> proc data) :
  (0 < j)%N -> (j < n_relay)%N ->
  relay_body j = Send 0 sv0 (Recv 0 f0) ->
  exists f_dec, f0 (e_local alice_c) = Recv j f_dec /\
    forall m rr, f_dec (e_local (enc (pub_of_priv (dk_relay j)) m rr)) =
      Send j.+2 (e_local (Emul_local alice_c
        (enc (ek (nat_to_party_id j.+2)) m (r2_relay j)))) Finish.
Proof.
move=> Hj0 Hjn Hbody.
rewrite /relay_body (negbTE (lt0n_neq0 Hj0)) in Hbody.
have Hjn' : (j : nat) == n_relay = false
...
rewrite Hjn' /pRecvEnc_local /std_Recv_enc /Recv_param in Hbody.
case: Hbody => _ Hf0; subst f0; rewrite /comp.
rewrite std_from_enc_e_local /=.
rewrite /pRecvDec_local /std_Recv_dec /Recv_param.
eexists; split; first by [].
move=> m rr; rewrite /comp std_from_enc_e_local /= dec_correct /=.
by [].
Qed.
...
have [sk Hsend] := relay_body_is_send0 j.
...
| Inv_AR (j : 'I_n_relay.+1) ps (rr_fw : rand AHE) :
    ...
    ((j == 2%N :> nat) ->                                              (* A8: frontier sender for j=2 *)
       nth (default_proc data) ps 1 =
         Send 2 (e_local (enc (ek (nat_to_party_id 2)) (chain_acc 0) rr_fw)) Finish) ->
    ((3 <= j)%N ->                                                     (* A9: full frontier state *)
       (forall i : nat, (i.+1 < j.-2)%N ->
          nth (default_proc data) ps i.+1 = Finish) /\
       nth (default_proc data) ps j.-2 =
          Send j.-1 (e_local (enc (ek (nat_to_party_id j.-1)) (chain_acc (j - 3)) rr_fw)) Finish /\
       (exists f_recv, nth (default_proc data) ps j.-1 = Recv j.-2 f_recv /\
          (forall m rr, f_recv (e_local (enc (ek (nat_to_party_id j.-1)) m rr)) =
             Send j (e_local (Emul_local (alice_enc (inord j.-1))
               (enc (ek (nat_to_party_id j)) m (r2_relay (inord j.-2))))) Finish))) ->
    dsdp_inv ps
| Inv_AS0 ps (f_inner : plain AHE -> proc data) :
    size ps = n_relay.+2 ->
    @all_proc_wf AHE ps ->
    nth (default_proc data) ps 0 = Send 1 (e_local (alice_enc ord0)) (alice_foldr_at 1) ->
    nth (default_proc data) ps 1 =
...
| Inv_AS1 ps (f_inner : cipher AHE -> proc data) :
    size ps = n_relay.+2 ->
    @all_proc_wf AHE ps ->
    nth (default_proc data) ps 0 = Send 1 (e_local (alice_enc (inord 1))) (alice_foldr_at 2) ->
    nth (default_proc data) ps 1 =
    ...
    (n_relay = 1%N -> exists f, nth (default_proc data) ps 2 = Recv 1 f /\ (* H6b *)
       forall m rr, f (e_local (enc (ek (nat_to_party_id n_relay.+1)) m rr)) =
         Send 0 (e_local (enc (ek alice_idx) m (r2_relay (inord n_relay)))) Finish) ->
    dsdp_inv ps
| Inv_ASj (j : 'I_n_relay.+1) ps (rr_fw : rand AHE) :
    (2 <= j)%N ->                                                      (* B1 *)
    size ps = n_relay.+2 ->                                            (* B2 *)
    @all_proc_wf AHE ps ->                                            (* B3 *)
    nth (default_proc data) ps 0 =                                    (* B4 *)
       Send j (e_local (alice_enc j)) (alice_foldr_at j.+1) ->
    ...
    ((3 <= j)%N ->                                                     (* B9: Finish zone + frontier *)
       (forall i : nat, (i.+1 <= j.-2)%N ->
          nth (default_proc data) ps i.+1 = Finish) /\
       nth (default_proc data) ps j.-1 =
          Send j (e_local (enc (ek (nat_to_party_id j)) (chain_acc j.-2) rr_fw)) Finish) ->
    dsdp_inv ps
| Inv_drain (j : 'I_n_relay.+1) ps (rr_drain : rand AHE) :
    (j.+1 < n_relay.+1)%N ->
    size ps = n_relay.+2 ->
    @all_proc_wf AHE ps ->
    nth (default_proc data) ps 0 = alice_foldr_at n_relay.+1 ->
    nth (default_proc data) ps j.+1 =
      Send j.+2 (e_local (enc (ek (nat_to_party_id j.+2)) (chain_acc j) rr_drain)) Finish ->
    (exists f, nth (default_proc data) ps j.+2 = Recv j.+1 f) ->
    (forall i : nat, (i < j)%N -> nth (default_proc data) ps i.+1 = Finish) ->
    (exists f, nth (default_proc data) ps n_relay.+1 = Recv n_relay f /\  (* H8a: last relay *)
       forall m rr, f (e_local (enc (ek (nat_to_party_id n_relay.+1)) m rr)) =
         Send 0 (e_local (enc (ek alice_idx) m (r2_relay (inord n_relay)))) Finish) ->
    (forall i : nat, (j < i)%N -> (i < n_relay)%N ->                     (* H8b: between zone *)
       exists f, nth (default_proc data) ps i.+1 = Recv i f /\
         (forall m rr, f (e_local (enc (ek (nat_to_party_id i.+1)) m rr)) =
            Send i.+2 (e_local (Emul_local (alice_enc (inord i.+1))
              (enc (ek (nat_to_party_id i.+2)) m (r2_relay (inord i))))) Finish)) ->
    dsdp_inv ps
| Inv_tail ps (rr_tail : rand AHE) :
    size ps = n_relay.+2 ->
    @all_proc_wf AHE ps ->
...
Proof.
case.
- (* Inv_AR *) move=> j ps0 rr0 Hsz Hwf Halice Hbody Hpending _ _ _ _.
  have [sk Hrel] := relay_body_is_send0 j.
  ...
  exact (@has_comm_progress data ps0 j.+1 0 _ sk f Hj Hbody Halice').
- (* Inv_AS0 *) move=> ps0 f_inner Hsz Hwf Halice Hr0 Hpending _.
  have Hsz0 : (0 < size ps0)%N by rewrite Hsz.
  exact (@has_comm_progress data ps0 0 1 _ (alice_foldr_at 1)
    (oapp f_inner Fail \o (obind (@dec AHE (dk_relay ord0)) \o @std_from_enc AHE))
    Hsz0 Halice Hr0).
- (* Inv_AS1 *) move=> ps0 f_inner Hsz Hwf Halice Hr0 Hpending _ _ _.
  have Hsz0 : (0 < size ps0)%N by rewrite Hsz.
  exact (@has_comm_progress data ps0 0 1 _ (alice_foldr_at 2)
    (oapp f_inner Fail \o @std_from_enc AHE)
    Hsz0 Halice Hr0).
- (* Inv_ASj *) move=> j ps0 rr0 Hj Hsz Hwf Halice [sv0 [f0 [Hbody0 Hrj]]] Hpending _ _ _ _.
  have Hsz0 : (0 < size ps0)%N by rewrite Hsz.
  exact (@has_comm_progress data ps0 0 j _ (alice_foldr_at j.+1) f0 Hsz0 Halice Hrj).
- (* Inv_drain *) move=> j ps0 rr_drain Hjb Hsz Hwf Halice Hsend [f Hrecv] _ [_ _] _.
  have Hj : (j.+1 < size ps0)%N by rewrite Hsz; exact (ltn_trans Hjb (ltnSn _)).
  exact (@has_comm_progress data ps0 j.+1 j.+2 _ Finish f Hj Hsend Hrecv).
- (* Inv_tail *) move=> ps0 rr_tail Hsz Hwf Hsend Halice Hrels_fin.
  ...
  exact (@has_comm_progress data ps0 n_relay.+1 0 _ Finish f Hn Hsend Halice').
...
(*******************************************************************************)
(** *** Bridge to smc_deadlock.v: deadlock-freedom via acyclic wait-for        *)
(*******************************************************************************)

Definition mk_tup (ps : seq (proc data)) (Hsz : size ps = n_relay.+2) :
  (n_relay.+2).-tuple (proc data) := Tuple (introT eqP Hsz).

(* D1: target validity predicate in seq/nat world *)
Definition proc_target_valid (n : nat) (p : proc data) : Prop :=
  match p with
  | Send j _ _ => (j < n)%N
  | Recv j _ => (j < n)%N
  | _ => True
...
(* L1: relay body always targets 0 *)
Lemma relay_body_target_valid (j : 'I_n_relay.+1) :
  proc_target_valid n_relay.+2 (relay_body j).
Proof.
have [sk ->] := relay_body_is_send0 j. exact: ltn0Sn.
Qed.

(* L2: alice_foldr_at j targets j+1, valid when j < n_relay+1 *)
Lemma alice_foldr_target_valid (j : nat) (Hj : (j < n_relay.+1)%N) :
  proc_target_valid n_relay.+2 (alice_foldr_at j).
Proof.
rewrite /alice_foldr_at.
have [f ->] := @alice_body_at_recv j Hj.
rewrite /proc_target_valid. exact: ltn_ord (Ordinal Hj).
Qed.

(* L3: alice_foldr_at (n_relay+1) targets n_relay+1 *)
Lemma alice_tail_target_valid :
  proc_target_valid n_relay.+2 (alice_foldr_at n_relay.+1).
Proof.
rewrite alice_foldr_at_tail.
have [f ->] := alice_tail_is_recv. exact: ltnSn.
Qed.

(* L4: all targets valid in seq/nat world *)
Lemma dsdp_inv_all_targets_valid ps :
  dsdp_inv ps ->
  forall k, (k < size ps)%N ->
    proc_target_valid (size ps) (nth (default_proc data) ps k).
Proof.
case.
(* Inv_AR *)
- move=> j0 ps0 rr0 Hsz Hwf Halice Hbody Hpending H6 H7 H8 H9 k Hk.
  case: k Hk => [|k'] Hk; first by rewrite Halice Hsz; exact: alice_foldr_target_valid (ltn_ord j0).
  have Hk'b : (k' < n_relay.+1)%N by rewrite Hsz ltnS in Hk.
  case: (leqP (j0 : nat) k') => Hjk.
  + (* k' >= j0: relay body zone *)
    have Hpk : relay_at_body (Ordinal Hk'b) ps0.
      have [Heq|Hneq] := eqVneq k' (j0 : nat).
        subst k'. have -> : Ordinal Hk'b = j0 by exact: val_inj. exact: Hbody.
      have : (j0 < Ordinal Hk'b)%N by rewrite /= ltn_neqAle eq_sym Hneq Hjk.
      exact: Hpending _.
    rewrite /relay_at_body /= in Hpk. rewrite Hpk Hsz. exact: relay_body_target_valid.
  + (* k' < j0: frontier zone *)
    case: (ltngtP (j0 : nat) 2) => [Hj02|Hj02|Hj02].
    - (* j0 < 2 *)
      have : (j0 : nat) = 0%N \/ (j0 : nat) = 1%N.
        move: Hj02; case Hv: (j0 : nat) => [|[|n]] //; [left|right] => //.
      case=> Hj0v.
      + by rewrite Hj0v in Hjk.
      + have Hk0 : k' = 0%N by apply/eqP; rewrite -leqn0 -ltnS -Hj0v.
        subst k'.
        have Hj01 : j0 == 1%N :> nat by rewrite Hj0v.
        have [f_enc [Hp1 _]] := H6 Hj01.
        rewrite Hp1 /proc_target_valid Hsz. exact: ltn0Sn.
    - (* j0 > 2 *)
      have Hj2 : (1 < j0)%N by exact: ltn_trans _ Hj02.
      have [Hfin [Hsend_fw [f_recv [Hrecv_fw _]]]] := H9 Hj02.
      have [sv0 [f0 [_ Hrj0]]] := H7 Hj2.
      have Hbound : ((j0 : nat) < n_relay.+2)%N := ltn_trans (ltn_ord j0) (ltnSn _).
      have [Heqj|Hnej] := eqVneq k'.+1 (j0 : nat).
      + rewrite Heqj Hrj0 Hsz. exact: ltn0Sn.
      + have Hk'lt : (k'.+1 < j0)%N by rewrite ltn_neqAle Hnej -ltnS.
        have [Heqm1|Hnem1] := eqVneq k'.+1 (j0 : nat).-1.
        * rewrite Heqm1 Hrecv_fw /proc_target_valid Hsz.
          exact: leq_ltn_trans (leq_trans (leq_pred _) (leq_pred _)) Hbound.
        * have [Heqm2|Hnem2] := eqVneq k'.+1 (j0 : nat).-2.
          -- rewrite Heqm2 Hsend_fw /proc_target_valid Hsz.
             exact: leq_ltn_trans (leq_pred _) Hbound.
          -- have Hk'lt_m2 : (k'.+1 < (j0 : nat).-2)%N.
               move: Hk'lt Hnem1 Hnem2.
               case: (j0 : nat) Hj02 => [|[|[|j']]] //= _.
               rewrite !ltnS => Hle Hne1 Hne2.
               have Hne1' : k' != j'.+1 by move: Hne1; rewrite eqSS.
               have Hne2' : k' != j' by move: Hne2; rewrite eqSS.
               rewrite leq_eqVlt (negbTE Hne1') /= ltnS in Hle.
               by rewrite leq_eqVlt (negbTE Hne2') /= in Hle.
             rewrite (Hfin k' Hk'lt_m2). done.
    - (* j0 = 2 *)
      have Hj2 : (1 < j0)%N by rewrite Hj02.
      have [sv0 [f0 [_ Hrj0]]] := H7 Hj2.
      have [Heqj|Hnej] := eqVneq k'.+1 (j0 : nat).
      + rewrite Heqj Hrj0 Hsz. exact: ltn0Sn.
      + have Hk0 : k' = 0%N.
          move: Hjk; rewrite Hj02 ltnS leq_eqVlt.
          move/orP => [/eqP H'|H'].
          - exfalso; move: Hnej; rewrite Hj02 H'. done.
          - by move: H'; rewrite ltnS leqn0 => /eqP.
        subst k'.
        have Hj2eq : (j0 == 2%N :> nat) by rewrite Hj02.
        have Hp1 := H8 Hj2eq.
        rewrite Hp1 /proc_target_valid Hsz.
        by rewrite !ltnS.
(* Inv_AS0 *)
- move=> ps0 f_inner Hsz Hwf Halice Hr0 Hpending _ k Hk.
  case: k Hk => [|[|k'']] Hk.
  + rewrite Halice /= Hsz. exact: ltn_trans (ltnSn 0) (ltnSn _).
  + rewrite Hr0 Hsz. exact: ltn0Sn.
  + have Hk''b : (k''.+1 < n_relay.+1)%N by rewrite -ltnS -Hsz.
    have := Hpending (Ordinal Hk''b) (erefl : (0 < k''.+1)%N).
    rewrite /relay_at_body /= => ->. rewrite Hsz. exact: relay_body_target_valid.
(* Inv_AS1 *)
- move=> ps0 f_inner Hsz Hwf Halice Hr0 Hpending _ H6a H6b k Hk.
  case: k Hk => [|[|[|k''']]] Hk.
  + rewrite Halice /= Hsz. exact: ltn_trans (ltnSn 0) (ltnSn _).
  + rewrite Hr0 Hsz. exact: ltn0Sn.
  + case: (ltnP 1 n_relay) => Hnr1.
    * have [sv [f [_ Hp2]]] := H6a Hnr1. rewrite Hp2 Hsz. exact: ltn0Sn.
    * have Hnr : n_relay = 1%N by apply/eqP; rewrite eqn_leq Hn_relay Hnr1.
      have [f [Hp2 _]] := H6b Hnr. rewrite Hp2 Hsz. exact: ltn_trans (ltnSn 0) (ltnSn _).
  + have Hk'''b : (k'''.+2 < n_relay.+1)%N by rewrite -ltnS -Hsz.
    have := Hpending (Ordinal Hk'''b) (erefl : (1 < k'''.+2)%N).
    rewrite /relay_at_body /= => ->. rewrite Hsz. exact: relay_body_target_valid.
(* Inv_ASj *)
- move=> j0 ps0 rr0 Hj2 Hsz Hwf Halice [sv0 [f0 [_ Hrj]]] Hpending H7a H7b H8 H9 k Hk.
  case: k Hk => [|k'] Hk; first by rewrite Halice /= Hsz; exact: ltn_trans (ltn_ord j0) (ltnSn _).
  have Hk'b : (k' < n_relay.+1)%N by rewrite Hsz ltnS in Hk.
  case: (ltngtP k' (j0 : nat)) => [Hlt|Hgt|Heq].
  + (* k' < j0: frontier *)
    have Hbound : ((j0 : nat) < n_relay.+2)%N := ltn_trans (ltn_ord j0) (ltnSn _).
    have [Heqj|Hnej] := eqVneq k'.+1 (j0 : nat).
    * rewrite Heqj Hrj Hsz. exact: ltn0Sn.
    * have Hk'lt : (k'.+1 < j0)%N by rewrite ltn_neqAle Hnej -ltnS.
      case: (ltngtP (j0 : nat) 2) => [Hj02|Hj02|Hj02].
      -- exfalso; move: Hj02; rewrite ltnNge; apply/negP/negPn; exact: Hj2.
      -- have [Hfin Hsend_fw] := H9 Hj02.
         have [Heqm1|Hnem1] := eqVneq k'.+1 (j0 : nat).-1.
         ++ rewrite Heqm1 Hsend_fw /proc_target_valid Hsz. exact: Hbound.
         ++ have Hk'lt_m2 : (k' < (j0 : nat).-2)%N.
              move: Hk'lt Hnem1.
              case: (j0 : nat) Hj02 => [|[|[|j']]] //= _.
              rewrite !ltnS => Hle Hne1.
              have Hne1' : k' != j'.+1 by move: Hne1; rewrite eqSS.
              rewrite leq_eqVlt (negbTE Hne1') /= ltnS in Hle. exact: Hle.
            rewrite (Hfin k' Hk'lt_m2). done.
      -- have Hk0 : k' = 0%N by apply/eqP; rewrite -leqn0 -ltnS; move: Hk'lt; rewrite Hj02.
         subst k'.
         have Hj2eq : (j0 == 2%N :> nat) by rewrite Hj02.
         have Hp1 := H8 Hj2eq.
         rewrite Hp1 /proc_target_valid Hsz.
         by rewrite !ltnS.
  + (* k' > j0: pending relay body *)
    have Hgt' : (j0 < Ordinal Hk'b)%N by exact: Hgt.
    have Hpk := Hpending _ Hgt'.
    rewrite /relay_at_body /= in Hpk. rewrite Hpk Hsz. exact: relay_body_target_valid.
  + (* k' = j0: position j0+1, use B7a/B7b *)
    subst k'. case: (ltnP (j0 : nat) n_relay) => Hjn.
    -- have [sv' [f' [_ Hp]]] := H7a Hjn. rewrite Hp Hsz. exact: ltn0Sn.
    -- have Hjn' : (j0 : nat) = n_relay by apply/eqP; rewrite eqn_leq Hjn andbT -ltnS; exact (ltn_ord j0).
       have [f' [Hp _]] := H7b Hjn'. rewrite Hjn' Hp Hsz.
       exact: ltn_trans (ltnSn _) (ltnSn _).
(* Inv_drain *)
- move=> j0 ps0 rr0 Hjb Hsz Hwf Halice Hsend [f Hrecv] Hfinish [flast [Hlast _]] H8b k Hk.
  case: k Hk => [|k'] Hk; first by rewrite Halice Hsz; exact: alice_tail_target_valid.
  have Hk'b : (k' < n_relay.+1)%N by rewrite Hsz ltnS in Hk.
  case: (ltngtP k' (j0 : nat)) => [Hlt|Hgt|Heq].
  + (* k' < j0: Finish *) by rewrite (Hfinish k' Hlt).
  + (* k' > j0 *)
    have [Heq1 | Hne1] := eqVneq k' (j0 : nat).+1.
    * (* k' = j0+1: Recv (j0+1) f *) subst k'. rewrite Hrecv Hsz.
      exact: ltn_trans Hjb (ltnSn _).
    * (* k' > j0+1 *)
      have [Heqn | Hnen] := eqVneq k' n_relay.
      -- (* k' = n_relay: Recv n_relay f *) subst k'. rewrite Hlast Hsz.
         exact: ltn_trans (ltnSn _) (ltnSn _).
      -- (* j0+1 < k' < n_relay: Recv k' f *)
         have Hk'lt : (k' < n_relay)%N by rewrite ltn_neqAle Hnen -ltnS.
         have Hk'gt : (j0 < k')%N := Hgt.
         have [fk [Hpk _]] := H8b k' Hk'gt Hk'lt.
         rewrite Hpk Hsz. exact: ltn_trans Hk'lt (ltn_trans (ltnSn _) (ltnSn _)).
  + (* k' = j0: Send (j0+2) v Finish *) subst k'. rewrite Hsend Hsz. exact: Hjb.
(* Inv_tail *)
- move=> ps0 rr0 Hsz Hwf Hsend Halice Hfin k Hk.
  case: k Hk => [|k'] Hk; first by rewrite Halice Hsz; exact: alice_tail_target_valid.
  have Hk'b : (k' < n_relay.+1)%N by rewrite Hsz ltnS in Hk.
  have [Heq | Hne] := eqVneq k' n_relay.
  + by subst k'; rewrite Hsz Hsend.
  + have Hk'lt : (k' < n_relay)%N by rewrite ltn_neqAle Hne -ltnS.
    have := Hfin (Ordinal Hk'b) Hk'lt.
    by rewrite /relay_at_finish_pred /= => ->.
(* Inv_ret *)
- move=> ps0 d0 Hsz Hwf Hret Hfin k Hk.
  case: k Hk => [|k'] Hk; first by rewrite Hret.
  have Hk'b : (k' < n_relay.+1)%N by rewrite Hsz ltnS in Hk.
  have := Hfin (Ordinal Hk'b).
  by rewrite /relay_at_finish_pred /= => ->.
Qed.

(* L5: thin wrapper *)
Lemma dsdp_inv_wf_targets ps (Hsz : size ps = n_relay.+2) :
  dsdp_inv ps -> wf_targets (@mk_tup ps Hsz).
Proof.
move=> Hinv i. rewrite (tnth_nth (default_proc data)) /mk_tup /=.
have Hi : (nat_of_ord i < size ps)%N by rewrite Hsz; exact: ltn_ord.
have := @dsdp_inv_all_targets_valid _ Hinv _ Hi. by rewrite Hsz.
Qed.

Lemma dsdp_inv_acyclic ps (Hsz : size ps = n_relay.+2) :
  dsdp_inv ps -> wait_for_acyclic (@mk_tup ps Hsz).
Proof.
move=> Hinv; move: Hsz; case: Hinv.
(* Inv_AR *)
- move=> j0 ps0 rr0 Hsz0 Hwf Halice Hbody Hpending H6 H7 H8 H9 Hsz.
  pose rank := (fun (i : 'I_n_relay.+2) =>
    if (nat_of_ord i == 0)%N then 0%N
    else if (nat_of_ord i <= j0.+1)%N then (j0.+2 - nat_of_ord i)%N
    else 1%N).
  apply (@wait_for_ranking_acyclic _ _ _ rank).
  move=> i j; rewrite /wait_for /mk_tup (tnth_nth (default_proc data)) /=.
  move=> /andP [Hstuck /eqP Htgt].
  case Hi0 : (nat_of_ord i) => [|i'].
  + (* i=0: Alice = Recv(j0+1,...). Matched with relay at j0+1. Ready. *)
    exfalso; move/andP: Hstuck => [_ Hns]; apply (negP Hns).
    rewrite /smc_deadlock.is_ready /= /smc_interpreter.step Hi0.
    have [f Hrecv_f] := @alice_body_at_recv j0 (ltn_ord j0).
    rewrite Halice /alice_foldr_at Hrecv_f.
    have [sk Hbs] := relay_body_is_send0 j0.
    rewrite /relay_at_body in Hbody. rewrite Hbody Hbs. by rewrite eqxx.
  + have Hi' : (i' < n_relay.+1)%N by move: (ltn_ord i); rewrite Hi0 ltnS.
    have [Heqj | Hnej] := eqVneq i' j0.
    * (* i'=j0 → pos j0+1 = relay_body j0 = Send 0. Matched with Alice. Ready. *)
      exfalso; move/andP: Hstuck => [_ Hns]; apply (negP Hns).
      rewrite /smc_deadlock.is_ready /= /smc_interpreter.step Hi0 Heqj.
      have [sk Hbs] := relay_body_is_send0 j0.
      rewrite /relay_at_body in Hbody. rewrite Hbody Hbs.
      have [f Hrecv_f] := @alice_body_at_recv j0 (ltn_ord j0).
      rewrite Halice /alice_foldr_at Hrecv_f. by rewrite eqxx.
    * have [Hgtj | Hlej] := ltnP j0 i'.
      -- (* i' > j0: relay zone. relay_body = Send 0. Target 0. *)
         have Hpk := Hpending (Ordinal Hi') Hgtj.
         rewrite /relay_at_body /= in Hpk.
         have [sk Hbs] := relay_body_is_send0 (Ordinal Hi').
         rewrite Hi0 Hpk Hbs /= in Htgt.
         case: Htgt => Hjeq. rewrite /rank /= -Hjeq Hi0 /=.
         have -> : (i'.+1 <= j0.+1)%N = false by apply/negbTE; rewrite -ltnNge.
         done.
      -- (* i' < j0: frontier zone *)
         have Hltj : (i' < j0)%N by rewrite ltn_neqAle Hnej Hlej.
         have Hj1 : (0 < j0)%N by apply: leq_ltn_trans Hltj.
         have [Hj_eq1 | Hj_ne1] := eqVneq (j0 : nat) 1.
         ++ (* j0=1: i'=0. H6: ps[1]=Recv 0. Stuck, targets 0. *)
            have Hi'0 : i' = 0%N by move: Hltj; rewrite Hj_eq1 ltnS leqn0 => /eqP.
            have Hj1b : (j0 == 1 :> nat) by exact/eqP.
            have [f_enc [Hp1 _]] := H6 Hj1b.
            rewrite Hi0 Hi'0 Hp1 /= in Htgt. case: Htgt => Hjeq.
            rewrite /rank /= -Hjeq Hi0 Hi'0 /=. done.
         ++ (* j0 >= 2 *)
            have Hj2 : (2 <= j0)%N by case: (j0 : nat) Hj1 Hj_ne1 => [|[|]] //.
            have [sv0 [f0 [_ Hrj]]] := H7 Hj2.
            have [Hj_eq2 | Hj_ne2] := eqVneq (j0 : nat) 2.
            ** (* j0=2 *)
               have Hj2b : (j0 == 2 :> nat) by exact/eqP.
               have Hp1 := H8 Hj2b.
               have [Hi'eq0 | Hi'neq0] := eqVneq i' 0.
               --- (* pos 1 = Send 2 sv Finish. Stuck, targets 2. *)
                   rewrite Hi0 Hi'eq0 Hp1 /= in Htgt. case: Htgt => Hjeq.
                   rewrite /rank /= -Hjeq Hi0 Hi'eq0 /=. by rewrite Hj_eq2.
               --- (* i'=1: ps[2] = Recv 0 f0. Stuck, targets 0. *)
                   have Hi'1 : i' = 1%N.
                     move: Hltj Hi'neq0; rewrite Hj_eq2.
                     by case: (i') => [|[|]].
                   have Hrj2 : nth (default_proc data) ps0 2 = Recv 0 f0.
                     by rewrite -(Hj_eq2 : (j0 : nat) = 2%N).
                   rewrite Hi0 Hi'1 Hrj2 /= in Htgt. case: Htgt => Hjeq.
                   rewrite /rank /= -Hjeq Hi0 Hi'1 /=. by rewrite Hj_eq2.
            ** (* j0 >= 3 *)
               have Hj3 : (3 <= j0)%N
                 by case: (j0 : nat) Hj2 Hj_ne2 => [|[|[|]]] //.
               have [Hfin_zone [Hsend_f [f_recv [Hrecv_f _]]]] := H9 Hj3.
               have [Hi_fin | Hi_nofin] := ltnP i'.+1 j0.-2.
               --- (* Finish zone: is_final → not stuck → exfalso *)
                   exfalso; move/andP: Hstuck => [Hnf _]; apply (negP Hnf).
                   have := Hfin_zone i' Hi_fin.
                   by rewrite (tnth_nth (default_proc data)) /mk_tup /= Hi0 => ->.
               --- have [Hi_send | Hi_nosend] := eqVneq i'.+1 j0.-2.
                   +++ (* pos j0-2 = Send(j0-1). Matched w/ Recv at j0-1. Ready. *)
                       exfalso; move/andP: Hstuck => [_ Hns]; apply (negP Hns).
                       rewrite /smc_deadlock.is_ready /= /smc_interpreter.step Hi0.
                       rewrite Hi_send Hsend_f Hrecv_f.
                       by rewrite eqxx.
                   +++ have [Hi_recv | Hi_norecv] := eqVneq i'.+1 j0.-1.
                       *** (* pos j0-1 = Recv(j0-2). Matched w/ Send at j0-2. Ready. *)
                           exfalso; move/andP: Hstuck => [_ Hns]; apply (negP Hns).
                           rewrite /smc_deadlock.is_ready /= /smc_interpreter.step Hi0.
                           rewrite Hi_recv Hrecv_f Hsend_f.
                           by rewrite eqxx.
                       *** (* i'+1 = j0 (only remaining value in range). ps[j0]=Recv 0 f0. Target 0.
                              ps[0]=Send j0 vd ... → non-final. *)
                           have Hi_j0 : i'.+1 = (j0 : nat).
                             have Hj1pos : (0 < (j0 : nat).-1)%N.
                               by move: Hj3; case: {+}(j0 : nat) => [//|[//|[//|n]]].
                             have Hge : ((j0 : nat).-1 <= i'.+1)%N.
                               rewrite -(prednK Hj1pos).
                               by rewrite ltn_neqAle eq_sym Hi_nosend Hi_nofin.
                             have Hge2 : ((j0 : nat) <= i'.+1)%N.
                               rewrite -(prednK (_ : (0 < j0)%N)); last first.
                                 by move: Hj3; case: {+}(j0 : nat) => [//|[//|[//|n]]].
                               by rewrite ltn_neqAle eq_sym Hi_norecv Hge.
                             by apply/eqP; rewrite eqn_leq Hge2 Hltj.
                           rewrite Hi0 Hi_j0 Hrj /= in Htgt.
                           case: Htgt => Hjeq.
                           rewrite /rank /= -Hjeq Hi0 /=.
                           (* goal: (0 < if i' < j0.+1 then j0.+2 - i'.+1 else 1)%N *)
                           (* Hi_j0 : i'.+1 = j0, so i' < j0.+1 *)
                           have : (i' < j0.+1)%N by rewrite -Hi_j0.
                           move=> ->. rewrite Hi_j0.
                           have Hsub : ((j0 : nat).+2 - (j0 : nat) = 2)%N.
                             by rewrite -(@subnSK (j0 : nat) (j0 : nat).+2 (ltn_trans (ltnSn _) (ltnSn _))) subSnn.
                           by rewrite Hsub.
(* Inv_AS0: rank = nat_of_ord. All stuck relays target 0 < i'+1. *)
- move=> ps0 f_inner Hsz0 Hwf Halice Hr0 Hpending _ Hsz.
  apply (@wait_for_ranking_acyclic _ _ _ (@nat_of_ord n_relay.+2)).
  move=> i j; rewrite /wait_for /mk_tup (tnth_nth (default_proc data)) /=.
  move=> /andP [Hstuck /eqP Htgt].
  case Hi0 : (nat_of_ord i) => [|i'].
  + (* i=0: Alice = Send(1,vd,...). Ready → not stuck. *)
    exfalso; move/andP: Hstuck => [_ Hns]; apply (negP Hns).
    by rewrite /smc_deadlock.is_ready /= /smc_interpreter.step Hi0 Halice Hr0 eqxx.
  + (* i=i'+1: relay or Recv at 1. *)
    have Hi' : (i' < n_relay.+1)%N by move: (ltn_ord i); rewrite Hi0 ltnS.
    have [Heq0 | Hne0] := eqVneq i' 0.
    * (* i'=0: pos 1 = Recv 0 (...). Targets 0. Matched with Alice → ready. Not stuck. *)
      exfalso; move/andP: Hstuck => [_ Hns]; apply (negP Hns).
      by rewrite /smc_deadlock.is_ready /= /smc_interpreter.step Hi0 Heq0 Hr0 Halice eqxx.
    * (* i'>0: relay_body = Send 0 sv sk. Targets 0. *)
      ...
      rewrite Hi0 Hpk Hbs /= in Htgt.
      case: Htgt => Hjeq.
      (* j = 0, rank(0) = 0 < rank(i) = i'.+1 *)
      by rewrite /= -Hjeq.
(* Inv_AS1: rank = nat_of_ord. *)
- move=> ps0 f_inner Hsz0 Hwf Halice Hr0 Hpending Hfi H6a H6b Hsz.
  apply (@wait_for_ranking_acyclic _ _ _ (@nat_of_ord n_relay.+2)).
  move=> i j; rewrite /wait_for /mk_tup (tnth_nth (default_proc data)) /=.
  move=> /andP [Hstuck /eqP Htgt].
  case Hi0 : (nat_of_ord i) => [|i'].
  ...
  + have Hi' : (i' < n_relay.+1)%N by move: (ltn_ord i); rewrite Hi0 ltnS.
    ...
    * have Hi'pos : (0 < i')%N by case: (i') Hne0.
      ...
      -- (* i'=1 → pos 2 *)
         have [Hle | Hgt] := leqP n_relay 1.
         ++ (* n_relay = 1 *)
            have Hn1 : n_relay = 1%N by apply/eqP; rewrite eqn_leq Hle Hn_relay.
            have [f [Hp2 _]] := H6b Hn1.
            rewrite Hi0 Heq1 Hp2 /= in Htgt.
            case: Htgt => Hjeq.
            by rewrite /= -Hjeq.
         ++ (* n_relay > 1 *)
            have [sv [f [_ Hp2]]] := H6a Hgt.
            rewrite Hi0 Heq1 Hp2 /= in Htgt.
            case: Htgt => Hjeq.
            by rewrite /= -Hjeq.
      -- (* i' > 1: relay_body = Send 0. Target 0. *)
         have Hi'gt1 : (1 < i')%N.
           by case: (i') Hi'pos Hne1 => [|[|i'']] //.
         have Hpk := Hpending (Ordinal Hi') Hi'gt1.
         rewrite /relay_at_body /= in Hpk.
         have [sk Hbs] := relay_body_is_send0 (Ordinal Hi').
         rewrite Hi0 Hpk Hbs /= in Htgt.
         case: Htgt => Hjeq.
         by rewrite /= -Hjeq.
(* Inv_ASj *)
- move=> j0 ps0 rr0 Hj2 Hsz0 Hwf Halice [sv0 [f0 [_ Hrj]]]
         Hpending H7a H7b H8 H9 Hsz.
  pose rank := (fun (i : 'I_n_relay.+2) =>
    if (nat_of_ord i == 0)%N then 0%N
    else if (nat_of_ord i <= j0)%N then (j0.+1 - nat_of_ord i)%N
    else nat_of_ord i).
  apply (@wait_for_ranking_acyclic _ _ _ rank).
  move=> i j; rewrite /wait_for /mk_tup (tnth_nth (default_proc data)) /=.
  move=> /andP [Hstuck /eqP Htgt].
  case Hi0 : (nat_of_ord i) => [|i'].
  + (* i=0: Alice = Send(j0,...). ps[j0]=Recv 0 f0. dst=j0, frm=0. *)
    (* step: ps[j0]=Recv 0 f0. Check ps[j0] with dst=j0: Recv frm=0, 0==0. Ready. *)
    exfalso; move/andP: Hstuck => [_ Hns]; apply (negP Hns).
    rewrite /smc_deadlock.is_ready /= /smc_interpreter.step Hi0 Halice Hrj.
    by rewrite eqxx.
  + have Hi' : (i' < n_relay.+1)%N by move: (ltn_ord i); rewrite Hi0 ltnS.
    (* i'+1 = j0? *)
    have [Heq_j0m1 | Hne_j0m1] := eqVneq i'.+1 (j0 : nat).
    * (* pos j0 = Recv 0 f0. step: frm=0, ps[0]=Send j0. dst=j0, j0==j0. Ready. *)
      exfalso; move/andP: Hstuck => [_ Hns]; apply (negP Hns).
      rewrite /smc_deadlock.is_ready /= /smc_interpreter.step Hi0 Heq_j0m1.
      by rewrite Hrj Halice eqxx.
    * have [Hgtj | Hlej] := ltnP j0 i'.
      -- (* i' > j0: check if i' == j0 or i'+1 > j0+1 *)
         have Hgtj1 : (j0 < i')%N := Hgtj.
         have [Hi_j0p1 | Hne_j0p1] := eqVneq i' (j0 : nat).
         ++ (* i' = j0: pos j0+1. *)
            (* If j0 < n_relay, H7a gives Recv 0 f at j0+1. *)
            (* If j0 = n_relay, H7b gives Recv n_relay f_dec at n_relay+1. *)
            have [Hjn | Hjn_eq] := ltnP (j0 : nat) n_relay.
            ** have [sv [f [_ Hp]]] := H7a Hjn.
               rewrite Hi0 Hi_j0p1 Hp /= in Htgt.
               case: Htgt => Hjeq. rewrite /rank /= -Hjeq Hi0 /=.
               rewrite Hi_j0p1. have -> := ltnn (j0 : nat). done.
            ** have Hjn' : (j0 : nat) = n_relay.
                 by apply/eqP; rewrite eqn_leq Hjn_eq andbT -ltnS; exact (ltn_ord j0).
               have [f_dec [Hp _]] := H7b Hjn'.
               rewrite Hi0 Hi_j0p1 Hjn' Hp /= in Htgt.
               case: Htgt => Hjeq. rewrite /rank /= -Hjeq Hi0 /=.
               rewrite Hi_j0p1 Hjn'.
               have -> := leqnn n_relay. rewrite subSnn.
               case: (eqVneq n_relay 0) => [-> //| Hne0].
               have -> := ltnn n_relay. exact Hn_relay.
         ++ (* i' > j0 and i' != j0: relay zone. *)
            have Hpk := Hpending (Ordinal Hi') Hgtj.
            rewrite /relay_at_body /= in Hpk.
            have [sk Hbs] := relay_body_is_send0 (Ordinal Hi').
            rewrite Hi0 Hpk Hbs /= in Htgt.
            case: Htgt => Hjeq. rewrite /rank /= -Hjeq Hi0 /=.
            have -> : (i'.+1 <= j0)%N = false.
              apply/negbTE; rewrite -ltnNge ltnS. exact (ltnW Hgtj).
            done.
      -- (* i' <= j0, i'+1 != j0 *)
         have [Heq_j0 | Hne_j0] := eqVneq i' (j0 : nat).
         ++ (* i' = j0: pos j0+1 — use B7a/B7b *)
            subst i'.
            have [Hjn | Hjn_eq] := ltnP (j0 : nat) n_relay.
            ** have [sv' [f' [_ Hp]]] := H7a Hjn.
               rewrite Hi0 Hp /= in Htgt. case: Htgt => Hjeq.
               rewrite /rank /= -Hjeq Hi0 /=.
               have -> := ltnn (j0 : nat). done.
            ** have Hjn' : (j0 : nat) = n_relay.
                 by apply/eqP; rewrite eqn_leq Hjn_eq andbT -ltnS; exact (ltn_ord j0).
               have [f_dec [Hp _]] := H7b Hjn'.
               rewrite Hi0 Hjn' Hp /= in Htgt. case: Htgt => Hjeq.
               rewrite /rank /= -Hjeq Hi0 Hjn' /=.
               have -> := leqnn n_relay. rewrite subSnn.
               case: (eqVneq n_relay 0) => [-> //| Hne0].
               have -> := ltnn n_relay. exact Hn_relay.
         ++ (* i' < j0: frontier zone *)
            have Hltj : (i' < j0)%N by rewrite ltn_neqAle Hne_j0 Hlej.
         (* j0 >= 2: case on j0=2 or j0 >= 3 *)
         have [Hj_eq2 | Hj_ne2] := eqVneq (j0 : nat) 2.
         ++ (* j0=2: i' < 2 and i'+1 != 2, so i'=0. *)
            have Hi'0 : i' = 0%N.
              move: Hltj Hne_j0m1; rewrite Hj_eq2.
              by case: (i') => [//|[|n']] //=.
            have Hj2b : (j0 == 2 :> nat) by exact/eqP.
            have Hp1 := H8 Hj2b.
            rewrite Hi'0 in Hi0.
            rewrite Hi0 Hp1 /= in Htgt. case: Htgt => Hjeq.
            rewrite /rank /= -Hjeq Hi0 /=. by rewrite Hj_eq2.
         ++ (* j0 >= 3: use H9 for finish zone + frontier *)
            have Hj3 : (3 <= j0)%N
              by case: (j0 : nat) Hj2 Hj_ne2 => [|[|[|]]] //.
            have [Hfin_zone Hsend_f] := H9 Hj3.
            have [Hi_fin | Hi_nofin] := ltnP i'.+1 j0.-2.
            ** (* Finish zone *)
               exfalso; move/andP: Hstuck => [Hnf _]; apply (negP Hnf).
               have := Hfin_zone i' (ltnW Hi_fin).
               by rewrite (tnth_nth (default_proc data)) /mk_tup /= Hi0 => ->.
            ** have [Hi_send | Hi_nosend] := eqVneq i'.+1 j0.-1.
               --- (* ps[j0-1] = Send j0 sv Finish. Targets j0. *)
                   rewrite Hi0 Hi_send Hsend_f /= in Htgt. case: Htgt => Hjeq.
                   rewrite /rank /= -Hjeq Hi0 /=.
                   (* Discharge if-conditions *)
                   have -> : ((j0 : nat) == 0) = false.
                     by move: Hj3; case: {+}(j0 : nat) => [//|[//|[//|]]].
                   have -> := leqnn (j0 : nat). rewrite subSnn.
                   have -> : (i' < j0)%N = true.
                     have : (i'.+1 < j0)%N by rewrite Hi_send; move: Hj3; case: {+}(j0 : nat) => [//|[//|[//|n]]].
                     by move=> /ltnW; rewrite -ltnS.
                   rewrite Hi_send.
                   (* goal: 1 < j0.+1 - j0.-2, where j0 >= 3 *)
                   (* 1 < j0.+1 - j0.-2 when j0 >= 3 *)
                   (* j0.+1 = j0.-2 + 3. So j0.+1 - j0.-2 = 3. *)
                   have Hlt_pred : ((j0 : nat).-1 < (j0 : nat).+1)%N.
                     apply (leq_ltn_trans (leq_pred (j0 : nat))). exact: ltnSn.
                   rewrite subSn; last exact: leq_pred.
                   have Hj0pos : (0 < (j0 : nat))%N.
                     by move: Hj3; case: {+}(j0 : nat) => [//|[//|[//|n]]].
                   by rewrite -{1}(prednK Hj0pos) subSnn.
               --- (* i'.+1 != j0-1: derive i'.+1 <= j0-2 → Finish zone *)
                   have Hj0pos : (0 < (j0 : nat))%N.
                     by move: Hj3; case: {+}(j0 : nat) => [//|[//|[//|n]]].
                   have Hi_lt_j : (i'.+1 < j0)%N.
                     by rewrite ltn_neqAle Hne_j0m1 Hltj.
                   have Hi_le_jm1 : (i'.+1 <= (j0 : nat).-1)%N.
                     by rewrite -ltnS (prednK Hj0pos).
                   have Hi_j0m2 : (i'.+1 <= j0.-2)%N.
                     have Hj1pos : (0 < (j0 : nat).-1)%N.
                       by move: Hj3; case: {+}(j0 : nat) => [//|[//|[//|n]]].
                     by rewrite -ltnS (prednK Hj1pos) ltn_neqAle Hi_nosend Hi_le_jm1.
                   exfalso; move/andP: Hstuck => [Hnf _]; apply (negP Hnf).
                   have := Hfin_zone i' Hi_j0m2.
                   by rewrite (tnth_nth (default_proc data)) /mk_tup /= Hi0 => ->.
(* Inv_drain *)
- move=> j0 ps0 rr0 Hjb Hsz0 Hwf Halice Hsend [f Hrecv]
         Hfinish [flast [Hlast _]] H8b Hsz.
  pose rank := (fun (i : 'I_n_relay.+2) =>
    if (nat_of_ord i == 0)%N then n_relay.+3
    else nat_of_ord i).
  apply (@wait_for_ranking_acyclic _ _ _ rank).
  move=> i j; rewrite /wait_for /mk_tup (tnth_nth (default_proc data)) /=.
  move=> /andP [Hstuck /eqP Htgt].
  case Hi0 : (nat_of_ord i) => [|i'].
  + (* i=0: Alice=alice_foldr_at n_relay.+1=Recv(n_relay.+1,f). *)
    (* ps[n_relay+1]: need to determine. Stuck → targets some j. *)
    (* alice_foldr_at n_relay.+1 = alice_erase_tail = Recv(n_relay.+1,...) *)
    have [ft Htail] := alice_tail_is_recv.
    rewrite Hi0 Halice alice_foldr_at_tail Htail /= in Htgt.
    case: Htgt => Hjeq.
    rewrite /rank -Hjeq Hi0 /=. exact: leqnSn.
  + have Hi' : (i' < n_relay.+1)%N by move: (ltn_ord i); rewrite Hi0 ltnS.
    (* i=i'+1: determine ps[i'+1] *)
    have [Heq_j0 | Hne_j0] := eqVneq i' j0.
    * (* i'+1 = j0+1: Send(j0+2,v,Finish). dst=j0+2. *)
      (* ps[j0+2] = Recv(j0+1,f). frm=j0+1=i'+1. Step matches. Ready. *)
      exfalso; move/andP: Hstuck => [_ Hns]; apply (negP Hns).
      rewrite /smc_deadlock.is_ready /= /smc_interpreter.step Hi0 Heq_j0.
      rewrite Hsend Hrecv. by rewrite eqxx.
    * have [Heq_j0p1 | Hi_j0p1] := eqVneq i' j0.+1.
      -- (* i'+1 = j0+2: Recv(j0+1,f). frm=j0+1. *)
         (* ps[j0+1]=Send(j0+2,...). dst=j0+2=i'+1. Step matches. Ready. *)
         exfalso; move/andP: Hstuck => [_ Hns]; apply (negP Hns).
         rewrite /smc_deadlock.is_ready /= /smc_interpreter.step Hi0 Heq_j0p1.
         rewrite Hrecv Hsend. by rewrite eqxx.
      -- have [Hlt_j0 | Hge_j0] := ltnP i' j0.
         ++ (* i' < j0: ps[i'+1] = Finish. Final → not stuck. *)
            exfalso; move/andP: Hstuck => [Hnf _]; apply (negP Hnf).
            have := Hfinish i' Hlt_j0.
            by rewrite (tnth_nth (default_proc data)) /mk_tup /= Hi0 => ->.
         ++ (* i' > j0+1: Recv chain. *)
            have Hgt_j0p1 : (j0.+1 < i')%N.
              rewrite ltn_neqAle eq_sym Hi_j0p1 /=.
              by rewrite (ltn_neqAle j0 i') eq_sym Hne_j0 Hge_j0.
            have [Heq_nr | Hne_nr] := eqVneq i' n_relay.
            ** (* i' = n_relay: ps[n_relay+1] = Recv n_relay flast. Targets n_relay. *)
               rewrite Hi0 Heq_nr Hlast /= in Htgt.
               case: Htgt => Hjeq. rewrite /rank -Hjeq Hi0 Heq_nr /=.
               have -> := gtn_eqF Hn_relay. exact: ltnSn.
            ** (* j0+1 < i' < n_relay: H8b gives Recv(i',f) at ps[i'+1]. Target i'. *)
               have Hlt_nr : (i' < n_relay)%N by rewrite ltn_neqAle Hne_nr -ltnS.
               have [fi [Hpi _]] := H8b i' (ltnW Hgt_j0p1) Hlt_nr.
               rewrite Hi0 Hpi /= in Htgt.
               case: Htgt => Hjeq. rewrite /rank -Hjeq Hi0 /=.
               have -> : (i' == 0) = false.
                 apply gtn_eqF. exact (ltn_trans (ltn0Sn _) Hgt_j0p1).
               exact: ltnSn.
(* Inv_tail: no stuck processes — show is_stuck i = false for all i *)
- move=> ps0 rr0 Hsz0 Hwf Hsend Halice Hfin Hsz.
  apply (@wait_for_ranking_acyclic _ _ _ (@nat_of_ord n_relay.+2)).
  move=> i j; rewrite /wait_for /mk_tup (tnth_nth (default_proc data)) /=.
  move=> /andP [Hstuck _].
  (* Show is_stuck i = false: either final or ready *)
  exfalso; move/andP: Hstuck => [Hnf Hns].
  case Hi0 : (nat_of_ord i) => [|i'].
  + (* i=0: Alice=Recv(n+1), matched with Send(0) at n+1. Ready. *)
    apply (negP Hns).
...
(* Inv_ret: no stuck processes *)
- move=> ps0 d0 Hsz0 Hwf Hret Hfin Hsz.
  apply (@wait_for_ranking_acyclic _ _ _ (@nat_of_ord n_relay.+2)).
  move=> i j; rewrite /wait_for /mk_tup (tnth_nth (default_proc data)) /=.
  move=> /andP [Hstuck _].
  exfalso; move/andP: Hstuck => [Hnf Hns].
...
(* Inv_AR *)
- move=> j0 ps0 rr0 Hsz0 Hwf Halice Hbody Hpending H6 H7 H8 H9 Htgt Hsz Hi.
  ...
  + (* i=0: Alice = Recv(j0+1,...). Ready → not stuck → exfalso *)
    ...
    have [sk ->] := relay_body_is_send0 j0. by rewrite eqxx.
  + have Hi' : (i' < n_relay.+1)%N by move: (ltn_ord i); rewrite Hi0 ltnS.
    ...
    * (* frontier: i' < j0. Determine ps[i'+1], extract target, show ps[target] non-final *)
      ...
      -- (* j0 >= 2 *)
         ...
         ++ (* j0 >= 3 *)
            ...
            have [Hfin_zone [Hsend_fw [f_recv [Hrecv_fw _]]]] := H9 Hj3.
      ...
      -- (* j0 = 1 *)
         ...
         by rewrite Halice /alice_foldr_at; have [f ->] := @alice_body_at_recv (nat_of_ord j0) (ltn_ord j0).
(* Inv_AS0 *)
- move=> ps0 f_inner Hsz0 Hwf Halice Hr0 Hpending _ Htgt Hsz Hi.
  case Hi0 : (nat_of_ord i) => [|i'].
  + exfalso; move/andP: Hi => [_ Hns]; apply (negP Hns).
    by rewrite /smc_deadlock.is_ready /= /smc_interpreter.step Hi0 Halice Hr0 eqxx.
  + have Hi' : (i' < n_relay.+1)%N by move: (ltn_ord i); rewrite Hi0 ltnS.
    ...
    * have Hi'pos : (0 < i')%N by case: (i') Hne0.
      ...
      by rewrite Halice.
(* Inv_AS1 *)
- move=> ps0 f_inner Hsz0 Hwf Halice Hr0 Hpending _ H6a H6b Htgt Hsz Hi.
  case Hi0 : (nat_of_ord i) => [|i'].
  + exfalso; move/andP: Hi => [_ Hns]; apply (negP Hns).
    by rewrite /smc_deadlock.is_ready /= /smc_interpreter.step Hi0 Halice Hr0 eqxx.
  + have Hi' : (i' < n_relay.+1)%N by move: (ltn_ord i); rewrite Hi0 ltnS.
    ...
    * have [Heq1 | Hne1] := eqVneq i' 1.
      ...
      -- have Hi'pos : (1 < i')%N by case: (i') Hne0 Hne1 => [//|[//|]].
         ...
         by rewrite Halice.
(* Inv_ASj *)
- move=> j0 ps0 rr0 Hj2 Hsz0 Hwf Halice [sv0 [f0 [_ Hrj]]] Hpending H7a H7b H8 H9 Htgt Hsz Hi.
  case Hi0 : (nat_of_ord i) => [|i'].
  ...
  + have Hi' : (i' < n_relay.+1)%N by move: (ltn_ord i); rewrite Hi0 ltnS.
    ...
    * (* i' < j0: frontier zone. Same structure as Inv_AR *)
      ...
      -- (* j0 >= 3 *)
         ...
         have [Hfin_zone Hsend_fw] := H9 Hj3.
         have [Hi_fin | Hi_nofin] := ltnP i'.+1 (j0 : nat).-2.
         ++ exfalso; move/andP: Hi => [Hnf _]; apply (negP Hnf).
            have := Hfin_zone i' (ltnW Hi_fin).
            by rewrite (tnth_nth (default_proc data)) /mk_tup /= Hi0 => ->.
...
(* Inv_drain *)
- move=> j0 ps0 rr0 Hjb Hsz0 Hwf Halice Hsend [f Hrecv] Hfinish [flast [Hlast _]] H8b Htgt Hsz Hi.
  case Hi0 : (nat_of_ord i) => [|i'].
...
(* Inv_tail *)
- move=> ps0 rr0 Hsz0 Hwf Hsend Halice Hfin Htgt Hsz Hi.
...
Proof.
rewrite /smc_deadlock.has_progress.
move/existsP => [i Hi].
rewrite /is_ready /= in Hi.
apply (@step_i_has_progress data ps (nat_of_ord i)).
  by rewrite Hsz; exact: ltn_ord.
exact: Hi.
Qed.

Lemma dsdp_inv_has_progress_via_deadlock ps :
  dsdp_inv ps -> has_progress data ps.
Proof.
move=> Hinv.
have Hsz : size ps = n_relay.+2.
  by case: Hinv => [j0 ps0 rr0 Hsz|ps0 f Hsz|ps0 f Hsz|j0 ps0 rr0 _ Hsz|
                     j0 ps0 rr0 _ Hsz|ps0 rr0 Hsz|ps0 d0 Hsz].
have Hap := @acyclic_progress data n_relay.+2 (mk_tup ps Hsz)
  (@dsdp_inv_wf_targets ps Hsz Hinv)
  (@dsdp_inv_acyclic ps Hsz Hinv)
  (@dsdp_inv_no_orphan ps Hsz Hinv).
case: Hap => [Haf|Hprog].
- exact: dsdp_inv_has_progress.
- exact: has_progress_tuple_to_seq Hprog.
Qed.

(* C2 sub-lemmas — signatures match enriched dsdp_inv constructors *)

(* C2a: AR(j) → AS variant *)
Lemma dsdp_inv_step_AR (j : 'I_n_relay.+1) ps (rr_fw : rand AHE) :
...
have [sk Hbs] := relay_body_is_send0 j.
have Halice_recv : nth (default_proc data) ps 0 = Recv (Ordinal (ltn_ord j)).+1 f
  by rewrite Halice /alice_foldr_at Hrecv_f.
have Hrelay_send : nth (default_proc data) ps j.+1 =
  Send 0 (e_local (enc_local (ek (nat_to_party_id j.+1)) (v_relay j) (r1_relay j))) sk.
  rewrite /relay_at_body in Hbody; rewrite Hbody Hbs. done.
have [Hstep_j1 Hstep_0] :=
  @step_send_recv_match data ps j.+1 0 _ sk f Hrelay_send Halice_recv.
have Hc_sv := std_from_enc_e_local (enc_local (ek (nat_to_party_id j.+1)) (v_relay j) (r1_relay j)).
have Hfsv := @alice_recv_to_send_foldr j (ltn_ord j) f _ _ Hc_sv Hrecv_f.
...
- (* j = 0 → Inv_AS0 *)
  ...
  + have Hsz0 : (0 < size ps)%N by rewrite Hsz.
    ...
    rewrite /alice_send_dest /= /alice_foldr_at /alice_enc /enc_local.
    have -> : Ordinal (n:=n_relay.+1) (m:=0) (ltn_ord ord0) = ord0 :> 'I_n_relay.+1
      by apply /val_inj.
    by rewrite /= maxn0.
  ...
  + move=> i Hi; rewrite /relay_at_body.
    ...
    have [ski Hbsi] := relay_body_is_send0 i.
...
- case: (posnP j.-1) => Hj1.
  + (* j = 1 → Inv_AS1 *)
    ...
    * exact (@one_step_preserves_proc_wf ps Hwf).
    * have Hsz0 : (0 < size ps)%N by rewrite Hsz.
      rewrite (@nth_one_step data ps 0 Hsz0) Hstep_0 Hfsv /alice_enc /enc_local.
      have -> : Ordinal (n:=n_relay.+1) (m:=1) (ltn_ord jord) = inord 1 :> 'I_n_relay.+1
        by apply /val_inj; rewrite /= inordK.
      have -> : jord = inord 1 :> 'I_n_relay.+1
        by apply /val_inj; rewrite /= inordK.
      rewrite /alice_foldr_at.
      suff -> : (inord 1 : 'I_n_relay.+1).+1 = 2 :> nat by [].
      by rewrite inordK.
    ...
    * (* H6a: 1 < n_relay → relay 1 body link *)
      ...
      have Hsk_eq : sk = Recv 0 f1.
        rewrite Hord_eq in Hb1.
        by move: Hb1; rewrite Hbs => -[_ <-].
      by rewrite (@nth_one_step data ps 2 Hsz2) Hstep_j1 Hsk_eq.
    * (* H6b: n_relay = 1 → last relay with concrete continuation *)
      move=> Hn1_eq.
      have [sv_last [f_last [Hblast Hflast_cont]]] := relay_last_recv_concrete.
      have Hord_max_eq : jord = @ord_max n_relay
        by apply /val_inj; rewrite /= Hn1_eq.
      rewrite Hord_max_eq in Hbs Hbody Hrelay_send Hstep_j1.
  ...
  + (* j >= 2 *)
    ...
    (* Case split: j >= 3 needs rr_new from HE chain; j = 2 uses rr_fw *)
    case: (leqP 3 j) => [Hj3_main|Hj2_main].
    + (* j >= 3: compute rr_new from HE chain before applying Inv_ASj *)
      have := H9 Hj3_main.
      set sv_fw_old := e_local (enc (ek (nat_to_party_id j.-1))
        (chain_acc (j - 3)) rr_fw).
      move=> [Hfinzone [Hfrontier_sender
               [f_recv [Hfrontier_recv Hfrecv_cont]]]].
      have Hsucc : j.-2.+1 = j.-1
        by case: (j : nat) Hj3_main => [|[|[|n]]].
      ...
      have Hrecv_result := Hfrecv_cont (chain_acc (j - 3)%N) rr_fw.
      have [rr_a Ha] := alice_enc_value (inord j.-1).
      have Hek : (inord j.-1 : 'I_n_relay.+1).+1 = j.-1.+1 :> nat.
        rewrite inordK //; exact (leq_ltn_trans (leq_pred _) (ltn_ord j)).
      have Hj2_eq : j.-1.+1 = j :> nat
        by rewrite prednK //; exact (ltn_trans (ltn0Sn 0) Hj2).
      have Hj3_eq : (j - 3)%N = j.-2.-1.
        case: (j : nat) Hj3_main => [|[|[|n]]] // _.
        by rewrite !subSS subn0 /=.
      have Hchain : chain_acc (j - 3)%N + term (inord j.-1) = chain_acc j.-2
        by rewrite Hj3_eq; case: (j : nat) Hj3_main Hj2 => [|[|[|n]]] //= _ _.
      have [rr_new Hnew] :
        exists rr, Emul_local (alice_enc (inord j.-1))
          (enc (ek j) (chain_acc (j - 3)%N) (r2_relay (inord j.-2))) =
          enc (ek j) (chain_acc j.-2) rr.
        rewrite /Emul_local Ha !enc_curry_eq Hek Hj2_eq
                -(@Emul_addM AHE) -enc_curry_eq /=.
        rewrite GRing.addrC Hchain.
        by eexists.
      apply (Inv_ASj j _ rr_new).
      * (* B1: 2 <= j *) exact Hj2.
      * (* B2: size *) by rewrite (@size_one_step data).
      * (* B3: wf *) exact (@one_step_preserves_proc_wf ps Hwf).
      * (* B4 *) have Hsz0 : (0 < size ps)%N by rewrite Hsz.
        rewrite (@nth_one_step data ps 0 Hsz0) Hstep_0 Hfsv /alice_enc /enc_local.
        have -> : Ordinal (n:=n_relay.+1) (m:=j) (ltn_ord j) = j :> 'I_n_relay.+1
          by apply /val_inj.
        by [].
      * (* B5 *) exists sv7, f7; split; first exact Hbody7.
        ...
        rewrite (@nth_one_step data ps j Hszj) /smc_interpreter.step
                Hpsj Halice_recv.
        by [].
      * (* B6 *) move=> i Hi; rewrite /relay_at_body.
        have Hszi : (i.+1 < size ps)%N by rewrite Hsz; exact (ltn_ord i).
        rewrite (@nth_one_step data ps i.+1 Hszi) /smc_interpreter.step.
        have Hbi := Hpending i Hi; rewrite /relay_at_body in Hbi.
        have [ski Hbsi] := relay_body_is_send0 i.
      ...
      * (* B7b *) move=> Hjn_eq.
        ...
        have [sv_last [f_last [Hbs_last Hflast_cont]]] := relay_last_recv_concrete.
        ...
        -- exact Hflast_cont.
      * (* B8: vacuous since j >= 3 *)
        move=> /eqP Habs; exfalso.
        by move: Hj3_main; rewrite Habs.
      * (* B9: Finish zone + frontier with rr_new *)
        move=> Hj3.
        split.
        -- move=> i Hi.
        ...
        -- have Hszjm1 : (j.-1 < size ps)%N.
           ...
           rewrite (@nth_one_step data ps j.-1 Hszjm1) Hstep_front_recv
                   Hrecv_result Hnew.
           by [].
    + (* j = 2: use rr_fw directly *)
      apply (Inv_ASj j _ rr_fw).
      * (* B1: 2 <= j *) exact Hj2.
      * (* B2: size *) by rewrite (@size_one_step data).
      * (* B3: wf *) exact (@one_step_preserves_proc_wf ps Hwf).
      * (* B4 *) have Hsz0 : (0 < size ps)%N by rewrite Hsz.
      ...
      * (* B8: j == 2 → frontier sender at pos 1 — NOP *)
        ...
        have Hnop : (smc_interpreter.step ps [::] 1).2 = false.
          apply (@forwarding_step_nop ps 0
            (e_local (enc (ek (nat_to_party_id 2)) (chain_acc 0) rr_fw))).
          - exact Hfw.
          - move=> g Habsurd.
      ...
      * (* B9: vacuous since j < 3 *)
        move=> Hj3; exfalso.
        by have := leq_ltn_trans Hj3 Hj2_main.
...
have [Hstep0 Hstep1] :=
  @step_send_recv_match data ps 0 1 (e_local (alice_enc ord0)) (alice_foldr_at 1)
    (oapp f_inner Fail \o (obind (@dec AHE (dk_relay ord0)) \o @std_from_enc AHE))
    Halice Hr0.
have Hsfe : @std_from_enc AHE (e_local (alice_enc ord0)) = Some (alice_enc ord0)
  := std_from_enc_e_local _.
set c := alice_enc ord0.
...
have Hord1 : (1 < n_relay.+1)%N := Hn_relay.
apply (Inv_AR (Ordinal Hord1) _ (r2_relay ord0)). (* dummy rr_fw — A8/A9 vacuous for j=1 *)
- by rewrite (@size_one_step data).
- exact (@one_step_preserves_proc_wf ps Hwf).
...
- rewrite /relay_at_body.
  ...
  have [sk1 Hbs1] := relay_body_is_send0 (Ordinal Hord1).
...
Lemma dsdp_inv_step_AS1 ps (f_inner : cipher AHE -> proc data) :
  ...
  all_terminated (one_step_procs data ps) \/ dsdp_inv (one_step_procs data ps).
Proof.
move=> Hsz Hwf Halice Hr0 Hpending Hfinner_cont
  Hrelay1_inter Hrelay1_last; right.
have [Hstep0 Hstep1] := @step_send_recv_match data ps 0 1
  (e_local (alice_enc (inord 1))) (alice_foldr_at 2)
  (oapp f_inner Fail \o @std_from_enc AHE) Halice Hr0.
have Hsfe : @std_from_enc AHE (e_local (alice_enc (inord 1))) = Some (alice_enc (inord 1))
  := std_from_enc_e_local _.
...
- (* n_relay >= 2 → Inv_AR(2) *)
  ...
  (* HE chain: sv_r0 = enc(ek 2, chain_acc 0, rr_combined) *)
  have [rr_combined Hcombined_AR] :
    ...
    by eexists.
  apply (Inv_AR (Ordinal Hord2) _ rr_combined).
  + by rewrite (@size_one_step data).
  + exact (@one_step_preserves_proc_wf ps Hwf).
  + have Hsz0 : (0 < size ps)%N by rewrite Hsz.
    by rewrite (@nth_one_step data ps 0 Hsz0) Hstep0.
  + rewrite /relay_at_body.
    ...
    have [sk2 Hbs2] := relay_body_is_send0 (Ordinal Hord2).
  ...
  + (* A8: frontier sender at pos 1 for j=2 *)
    move=> _.
    have Hsz1 : (1 < size ps)%N by rewrite Hsz; exact (ltn_trans Hn_relay (ltnSn _)).
    rewrite (@nth_one_step data ps 1 Hsz1) Hr0_result Hfic /sv_r0 /c Hcombined_AR.
    by [].
  + by []. (* A9: 3<=2 → false *)
- (* n_relay = 1 → Inv_drain(0) *)
  ...
  + by [].
  + (* H8a: last relay concrete continuation — use Hlast_cont from strengthened H6b *)
    exists f_last; split.
    * have Hsz2 : (2 < size ps)%N by rewrite Hsz Hn1_eq.
      rewrite Hn1_eq.
      by rewrite (@nth_one_step data ps 2 Hsz2) /smc_interpreter.step Hr_last Hr0 /=.
...
Lemma dsdp_inv_step_ASj (j : 'I_n_relay.+1) ps (rr_fw : rand AHE) :
  (2 <= j)%N ->
  size ps = n_relay.+2 -> @all_proc_wf AHE ps ->
  nth (default_proc data) ps 0 = Send j (e_local (alice_enc j)) (alice_foldr_at j.+1) ->
  ...
  all_terminated (one_step_procs data ps) \/ dsdp_inv (one_step_procs data ps).
Proof.
move=> Hj2 Hsz Hwf Halice [sv0 [f0 [Hbody0 Hrj]]] Hpending B7a B7b B8 B9;
  right.
have [Hstep0 Hstepj] := @step_send_recv_match data ps 0 j (e_local (alice_enc j))
  (alice_foldr_at j.+1) f0 Halice Hrj.
have Henc_vd : @std_from_enc AHE (e_local (alice_enc j)) != None
  by rewrite std_from_enc_e_local.
...
have [f_dec [Hf0vd Hf_dec_cont]] :=
  @relay_inter_recv_from_body_fires (inord j.-1) (e_local (alice_enc j)) sv0 f0
...
- (* Case 1: j < n_relay → Inv_AR(j+1) *)
  ...
  apply (Inv_AR (Ordinal Hord_j1) _ rr_fw).
  + by rewrite (@size_one_step data).
  + exact (@one_step_preserves_proc_wf ps Hwf).
  + have Hsz0 : (0 < size ps)%N by rewrite Hsz.
    by rewrite (@nth_one_step data ps 0 Hsz0) Hstep0.
  + rewrite /relay_at_body /=.
    ...
    have [sk_j1 Hbs_j1] := relay_body_is_send0 (Ordinal Hord_j1).
  ...
  + (* A9: 3 <= j+1 → Finish zone + frontier + receiver *)
    ...
    * (* A9b: frontier sender at pos j.-1 — NOP, passes through B8/B9 value *)
      case: (leqP 3 j) => Hj3.
      -- have [_ Hfrontier] := B9 Hj3.
         have Hszjm1 : (j.-1 < size ps)%N.
      ...
      -- have Hj_eq : (j : nat) = 2%N.
         ...
         by [].
    * (* A9c: frontier receiver at pos j — concrete Emul_local continuation *)
      (* Use relay_inter_recv_concrete instead of relay_inter_recv_from_body_fires *)
      have [f_dec2 [Hf0vd2 Hf_dec2_cont]] :=
        @relay_inter_recv_concrete (inord j.-1) (alice_enc j) sv0 f0
          Hj1_pos Hj1_lt_nr Hbody0.
      (* f_dec2 = f_dec since both come from f0 (e_local (alice_enc j)) *)
      have Hf_eq : f_dec = f_dec2
        by rewrite Hf0vd in Hf0vd2; case: Hf0vd2.
      subst f_dec2.
      exists f_dec.
      have Hszj : (j < size ps)%N
      ...
      -- rewrite (@nth_one_step data ps j Hszj) Hstepj Hf0vd.
         by rewrite Hj1_val.
      -- (* Concrete continuation from relay_inter_recv_concrete *)
         move=> m rr.
         have Hkey' : ek (nat_to_party_id j) = pub_of_priv (dk_relay (inord j.-1)).
           have H := key_relay (inord j.-1).
           have Heq : (inord j.-1 : 'I_n_relay.+1).+1 = j :> nat
             by rewrite Hj1_val (prednK Hj0).
           by rewrite Heq in H.
         rewrite Hkey'.
         have H := Hf_dec2_cont m rr.
         have Hval_eq : (inord j.-1 : 'I_n_relay.+1).+2 = j.+1 :> nat
           by rewrite Hj1_val (prednK Hj0).
         rewrite Hval_eq in H; rewrite H.
         have Hinordj : inord j = j :> 'I_n_relay.+1
           by apply /val_inj; rewrite /= inordK //; exact (ltn_trans Hjn (ltnSn _)).
         by rewrite Hinordj.
- (* Case 2: j >= n_relay → j = n_relay → Inv_drain *)
  ...
  + (* j >= 3: B9 gives Finish zone + frontier *)
    have [Hfin_zone Hfrontier] := B9 Hj3.
    ...
    apply (Inv_drain (Ordinal Hn2_lt) _ rr_fw).
    ...
    * (* Frontier sender: one_step[n_relay.-1] — NOP since receiver is at pos j = n_relay *)
      have Hszjm1 : (j.-1 < size ps)%N.
        rewrite Hsz; exact (ltn_trans (leq_ltn_trans (leq_pred j) (ltn_ord j)) (ltnSn n_relay.+1)).
      rewrite Hjn_eq in Hfrontier.
      (* Rewrite Hfrontier: nth ps n_relay.-1 = Send n_relay (enc ...) Finish *)
      (* The step at n_relay.-1: Send n_relay → need Recv at ps[n_relay] with src=n_relay.-1 *)
      (* But ps[n_relay] = Recv 0 f0 (from Hrj with j = n_relay), src=0 != n_relay.-1 → NOP *)
      have Hnop : (smc_interpreter.step ps [::] n_relay.-1).2 = false.
        rewrite /smc_interpreter.step Hfrontier.
        rewrite Hjn_eq in Hrj; rewrite Hrj /=.
        suff -> : (0%N == n_relay.-1) = false by [].
        by apply /eqP; move: Hn_ge3; case: (n_relay) => [|[|[|?]]].
      have H_ord_s1 : (Ordinal Hn2_lt : 'I_n_relay.+1).+1 = n_relay.-1 :> nat
        by rewrite /= Heq_s1.
      have H_ord_s2 : (Ordinal Hn2_lt : 'I_n_relay.+1).+2 = n_relay :> nat
        by rewrite /= Heq_s2.
      rewrite Hjn_eq in Hszjm1.
      rewrite H_ord_s1 (@nth_one_step_nop ps n_relay.-1 Hszjm1 Hnop) Hfrontier.
      by rewrite (prednK Hn_relay).
    ...
    * (* Frontier receiver: one_step[n_relay] = Recv n_relay.-1 f_dec *)
      ...
      have Hstepn : (step ps [::] n_relay).1.1 = f0 (e_local (alice_enc j)) by rewrite -Hjn_eq.
    ...
    * (* Last relay: one_step[n_relay.+1] = Recv n_relay f_last *)
      ...
      rewrite -Hjn_eq Hrj /=.
      by [].
    * (* Between zone: n_relay.-2 < i < n_relay, so i = j.-1 *)
      move=> /= i Hi1 Hi2.
      have Hi_eq : i = n_relay.-1.
        apply /eqP; rewrite eqn_leq.
        rewrite -Heq_s1 Hi1 /= andbT Heq_s1.
        by rewrite -ltnS (prednK Hn_relay).
      (* Convert i to j terms: i = j.-1, i.+1 = j, i.+2 = j.+1 *)
      have Hi_jm1 : i = j.-1 by rewrite Hi_eq Hjn_eq.
      have Hi1_j : i.+1 = j :> nat by rewrite Hi_jm1 (prednK Hj0).
      have Hi2_j1 : i.+2 = j.+1 :> nat by rewrite Hi1_j.
      have Hinord_i : inord i.+1 = j :> 'I_n_relay.+1.
        by apply /val_inj; rewrite /= inordK // Hi1_j; exact (ltn_ord j).
      have Hinord_i0 : inord i = inord j.-1 :> 'I_n_relay.+1.
        apply /val_inj; rewrite /= inordK; last first.
        - rewrite Hi_eq; exact (leq_ltn_trans (leq_pred _) (ltnSn _)).
        - rewrite inordK //; exact Hi_jm1.
      exists f_dec; split.
      -- rewrite Hi_eq (prednK Hn_relay).
         have Hszn : (n_relay < size ps)%N
           by rewrite Hsz; exact (ltn_trans (ltnSn n_relay) (ltnSn n_relay.+1)).
         rewrite Hjn_eq in Hf0vd.
         have Hstepn : (step ps [::] n_relay).1.1 = f0 (e_local (alice_enc j)) by rewrite -Hjn_eq.
         rewrite Hjn_eq in Hj1_val.
         by rewrite (@nth_one_step data ps n_relay Hszn) Hstepn Hf0vd Hj1_val.
      -- (* Between zone receiver continuation *)
         have [f_dec2 [Hf0vd2 Hf_dec2_cont]] :=
           @relay_inter_recv_concrete (inord j.-1) (alice_enc j) sv0 f0
             Hj1_pos Hj1_lt_nr Hbody0.
         have Hf_eq : f_dec = f_dec2
         ...
         subst f_dec2.
         move=> m rr.
         (* The goal has i-indexed terms; Hf_dec2_cont has (inord j.-1)-indexed terms.
            Strategy: rewrite Hf_dec2_cont indices to match goal using i = j.-1. *)
         have Hkey' : ek (nat_to_party_id j) = pub_of_priv (dk_relay (inord j.-1)).
           have H := key_relay (inord j.-1).
           have Heq : (inord j.-1 : 'I_n_relay.+1).+1 = j :> nat
             by rewrite Hj1_val (prednK Hj0).
           by rewrite Heq in H.
         (* nat_to_party_id i.+1 was partially reduced — fold it back *)
         rewrite -/(nat_to_party_id i.+1) -/(nat_to_party_id i.+2).
         have Hkey_eq : nat_to_party_id i.+1 = nat_to_party_id j by rewrite Hi1_j.
         rewrite Hkey_eq Hkey'.
         have H := Hf_dec2_cont m rr.
         have Hval_eq : (inord j.-1 : 'I_n_relay.+1).+2 = j.+1 :> nat
           by rewrite Hj1_val (prednK Hj0).
         rewrite Hval_eq in H; rewrite H.
         (* Unify by rewriting RHS i-terms to j-terms *)
         by rewrite Hi2_j1 Hinord_i Hinord_i0.
  + (* j = 2: B8 gives pos 1 = Send 2 sv Finish *)
    ...
    apply (Inv_drain (Ordinal Hord0) _ rr_fw).
    * by rewrite /=; exact Hn_relay.
    * by rewrite (@size_one_step data).
    * exact (@one_step_preserves_proc_wf ps Hwf).
    * have Hsz0 : (0 < size ps)%N by rewrite Hsz.
      by rewrite (@nth_one_step data ps 0 Hsz0) Hstep0 Hjn_eq2.
    * (* Frontier sender: one_step[1] = Send 2 concrete Finish — NOP *)
      have Hsz1 : (1 < size ps)%N
        by rewrite Hsz; exact (ltn_trans Hn_relay (ltnSn _)).
      have Hnop : (smc_interpreter.step ps [::] 1).2 = false.
        apply (@forwarding_step_nop ps 0 (e_local (enc (ek (nat_to_party_id 2)) (chain_acc 0) rr_fw))).
        - exact Hfrontier.
        - move=> g Habsurd.
          have Hj_is2 : (j : nat) = 2 by apply /eqP.
          by rewrite Hj_is2 in Hrj; rewrite Hrj in Habsurd.
      by rewrite (@nth_one_step_nop ps 1 Hsz1 Hnop) Hfrontier.
    ...
    * (* Between zone: 0 < i < n_relay = 2, only i = 1 *)
      ...
      have Hi_eq : i = 1.
        ...
        by rewrite /= in Hi1.
      (* Convert i to j terms: i = j.-1, i.+1 = j, i.+2 = j.+1 *)
      have Hi_jm1 : i = j.-1 by rewrite Hi_eq Hj_eq.
      have Hi1_j : i.+1 = j :> nat by rewrite Hi_jm1 (prednK Hj0).
      have Hi2_j1 : i.+2 = j.+1 :> nat by rewrite Hi1_j.
      have Hinord_i : inord i.+1 = j :> 'I_n_relay.+1.
        by apply /val_inj; rewrite /= inordK // Hi1_j; exact (ltn_ord j).
      have Hinord_i0 : inord i = inord j.-1 :> 'I_n_relay.+1.
        apply /val_inj; rewrite /= inordK; last first.
        - rewrite Hi_eq ltnS; exact Hn_relay.
        - rewrite inordK //; exact Hi_jm1.
      exists f_dec; split.
      -- have Hszj : (j < size ps)%N by rewrite Hsz Hj_eq Hn_eq2.
         have Hgoal3 : nth (default_proc data) (one_step_procs data ps) j = Recv j.-1 f_dec
           by rewrite (@nth_one_step data ps j Hszj) Hstepj Hf0vd Hj1_val.
         rewrite Hi_eq; rewrite Hj_eq in Hgoal3; exact Hgoal3.
      -- (* Between zone receiver continuation *)
         have [f_dec2 [Hf0vd2 Hf_dec2_cont]] :=
           @relay_inter_recv_concrete (inord j.-1) (alice_enc j) sv0 f0
             Hj1_pos Hj1_lt_nr Hbody0.
...
Lemma dsdp_inv_step_drain (j : 'I_n_relay.+1) ps (rr_drain : rand AHE) :
  (j.+1 < n_relay.+1)%N ->
  size ps = n_relay.+2 -> @all_proc_wf AHE ps ->
  nth (default_proc data) ps 0 = alice_foldr_at n_relay.+1 ->
  nth (default_proc data) ps j.+1 =
  ...
  (exists f, nth (default_proc data) ps j.+2 = Recv j.+1 f) ->
  (forall i : nat, (i < j)%N -> nth (default_proc data) ps i.+1 = Finish) ->
  (exists f, nth (default_proc data) ps n_relay.+1 = Recv n_relay f /\   (* H8a *)
     forall m rr, f (e_local (enc (ek (nat_to_party_id n_relay.+1)) m rr)) =
       Send 0 (e_local (enc (ek alice_idx) m (r2_relay (inord n_relay)))) Finish) ->
  (forall i : nat, (j < i)%N -> (i < n_relay)%N ->                     (* H8b *)
     exists f, nth (default_proc data) ps i.+1 = Recv i f /\
       (forall m rr, f (e_local (enc (ek (nat_to_party_id i.+1)) m rr)) =
          Send i.+2 (e_local (Emul_local (alice_enc (inord i.+1))
            (enc (ek (nat_to_party_id i.+2)) m (r2_relay (inord i))))) Finish)) ->
  all_terminated (one_step_procs data ps) \/ dsdp_inv (one_step_procs data ps).
Proof.
move=> Hjb Hsz Hwf Halice_foldr Hsend [f Hrecv] Hfin
  [fl [Hlast Hlast_cont]] Hbetween.
have Halice_save := Halice_foldr.
set vd := e_local (enc (ek (nat_to_party_id j.+2)) (chain_acc j) rr_drain).
...
- (* j+1 < n_relay: intermediate → Inv_drain(j+1) *)
  ...
  (* Apply concrete H8b continuation *)
  have Hfv := Hcont1 (chain_acc j) rr_drain.
  (* Hfv : f (e_local (enc (ek j.+2) (chain_acc j) rr_drain)) =
       Send j.+3 (e_local (Emul_local (alice_enc (inord j.+2)) (enc (ek j.+3) (chain_acc j) (r2_relay (inord j.+1))))) Finish *)
  have Hfv_vd : f vd = Send j.+3
    (e_local (Emul_local (alice_enc (inord j.+2))
      (enc (ek (nat_to_party_id j.+3)) (chain_acc j) (r2_relay (inord j.+1))))) Finish
    by rewrite /vd in Hfv.
  (* HE chain: derive enc form *)
  have Hj2_lt : (j.+2 < n_relay.+1)%N by rewrite ltnS; exact Hjn.
  have [rr_new Hnew] : exists rr,
    Emul_local (alice_enc (inord j.+2))
      (enc (ek (nat_to_party_id j.+3)) (chain_acc j) (r2_relay (inord j.+1))) =
      enc (ek (nat_to_party_id j.+3)) (chain_acc j.+1) rr.
    have [rr_a Ha] := alice_enc_value (inord j.+2).
    have Hek : (inord j.+2 : 'I_n_relay.+1).+1 = j.+3 :> nat
      by rewrite inordK.
    rewrite /Emul_local Ha !enc_curry_eq Hek -(@Emul_addM AHE) -enc_curry_eq.
    rewrite /chain_acc GRing.addrC.
    by eexists.
  have Hj1b : (j.+1 < n_relay.+1)%N := ltn_trans Hjn (ltnSn _).
  apply (Inv_drain (Ordinal Hj1b) _ rr_new).
  + by rewrite /= ltnS.
  + by rewrite (@size_one_step data).
  + exact (@one_step_preserves_proc_wf ps Hwf).
  ...
  + (* Relay j+1 forwarding: one_step[j+2] = Send j+3 (enc ... chain_acc j.+1 ...) Finish *)
    have Hszj2 : (j.+2 < size ps)%N by rewrite Hsz; exact (ltn_trans Hjn (ltnSn _)).
    rewrite (@nth_one_step data ps j.+2 Hszj2) Hstep_j1 Hfv_vd Hnew.
    by [].
  + (* Relay j+2 at Recv *)
...
- (* j+1 >= n_relay: last relay reached → Inv_tail *)
  ...
  subst fl.
  (* After firing: relay j → Finish, last relay → f vd = Send 0 enc Finish *)
  (* Apply concrete H8a continuation *)
  have Hfv := Hlast_cont (chain_acc j) rr_drain.
  (* Hfv : f (e_local (enc (ek n_relay.+1) (chain_acc j) rr_drain)) =
       Send 0 (e_local (enc (ek alice_idx) (chain_acc j) (r2_relay (inord n_relay)))) Finish *)
  have Hfv_vd : f vd = Send 0
    (e_local (enc (ek alice_idx) (chain_acc j) (r2_relay (inord n_relay)))) Finish.
    rewrite /vd; have -> : nat_to_party_id j.+2 = nat_to_party_id n_relay.+1 by rewrite Hjn_eq.
    exact Hfv.
  (* chain_acc j = chain_acc n_relay.-1 since j.+1 = n_relay → j = n_relay.-1 *)
  have Hj_eq_nm1 : j = n_relay.-1 :> nat.
    by have := Hjn_eq; rewrite -[in RHS](prednK Hn_relay) => -[].
  apply (Inv_tail (one_step_procs data ps) (r2_relay (inord n_relay))).
  + by rewrite (@size_one_step data).
  + exact (@one_step_preserves_proc_wf ps Hwf).
  + (* Last relay becomes Send 0 concrete Finish *)
    have Hszn1 : (n_relay.+1 < size ps)%N by rewrite Hsz.
    have Hszj2 : (j.+2 < size ps)%N by rewrite Heq_pos.
    rewrite -Heq_pos (@nth_one_step data ps j.+2 Hszj2) Hstep_j1 Hfv_vd Hj_eq_nm1.
    by [].
...
Lemma dsdp_inv_step_TAIL ps (rr_tail : rand AHE) :
  size ps = n_relay.+2 -> @all_proc_wf AHE ps ->
  nth (default_proc data) ps n_relay.+1 =
    Send 0 (e_local (enc (ek alice_idx) (chain_acc n_relay.-1) rr_tail)) Finish ->
  nth (default_proc data) ps 0 = alice_foldr_at n_relay.+1 ->
  ...
  all_terminated (one_step_procs data ps) \/ dsdp_inv (one_step_procs data ps).
Proof.
move=> Hsz Hwf Hsend Halice Hrels; right.
...
case.
- (* Inv_AR *)
  move=> j ps0 rr0 Hsz Hwf Halice Hbody Hpending H6 H7 H8 H9.
  exact: (dsdp_inv_step_AR j ps0 rr0 Hsz Hwf Halice Hbody Hpending H6 H7 H8 H9).
- (* Inv_AS0 *)
...
- (* Inv_ASj *)
  move=> j ps0 rr0 Hj Hsz Hwf Halice Hrecv Hpending B7a B7b B8 B9.
  exact: (dsdp_inv_step_ASj j ps0 rr0 Hj Hsz Hwf Halice Hrecv Hpending B7a B7b B8 B9).
- (* Inv_drain *)
  move=> j ps0 rr_drain Hjb Hsz Hwf Halice Hsend Hrecv Hfin Hlast Hbetween.
  exact: (dsdp_inv_step_drain j ps0 rr_drain Hjb Hsz Hwf Halice Hsend Hrecv Hfin Hlast Hbetween).
- (* Inv_tail *)
  move=> ps0 rr_tail Hsz Hwf Hsend Halice Hrels.
  exact: (dsdp_inv_step_TAIL ps0 rr_tail Hsz Hwf Hsend Halice Hrels).
- (* Inv_ret *)
...
have Halice : nth (default_proc data) ps2 0 = alice_foldr_at 0.
  ...
  reflexivity.
have Hord0 : (0 < n_relay.+1)%N by [].
eapply (Inv_AR (Ordinal Hord0) _ (r2_relay ord0)). (* dummy rr_fw — A8/A9 vacuous for j=0 *)
