...
security (factoring hardness), operational security (avoiding non-invertible elements 
that could leak factorization), and information-theoretic validity (field approximation). 
...
\textbf{Operational} & $\frac{\varphi(pq)}{pq} \approx 1$ & Random elements are invertible; avoids decryption ambiguities \\[0.5em]
...
\item \textbf{Fousse et al. (2011):} ``Large primes also prevent operational failures''
\item \textbf{This work (2025):} ``Large primes also validate information-theoretic analysis''
\end{itemize}
...
Non-invertible elements cause decryption ambiguity & Non-invertible elements invalidate field approximation \\[0.5em]
...
\bottomrule
\end{tabular}
\caption{Comparison of the two approaches to handling non-invertible elements}
\end{table}

\subsection{Key Insights}

\begin{enumerate}
\item \textbf{Different concerns, same root cause:} Both issues stem from non-invertible elements in $\mathbb{Z}/(pq)$

\item \textbf{Complementary solutions:}
\begin{itemize}
\item \textbf{Small primes ($< 2^{128}$):} Must use Fousse et al.'s corrected conditions (field approximation invalid)
\item \textbf{Cryptographic primes ($\geq 2^{1024}$):} Both concerns become negligible; can use field approximation
\end{itemize}
...
\item Both operational correctness and analytical validity are ensured
\end{itemize}
\end{enumerate}
...
\item \textbf{Three-fold security:} The requirement for large primes simultaneously 
  ...
  \item Operational security (avoiding non-invertible elements per Fousse et al.)
