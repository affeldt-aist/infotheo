...
Let next_graphs l :=
  [seq let (c,r) := graph_of_trace lam rho pns in (pns.1,c,r)
  | pns in build_traces def_port l.*2.+1].

Let lr k := if k is kv then lam else rho.

(*
Lemma weighted_count_step_it_is_tree_dist k c r p (t : tree 1 k) :
  r > 0 ->
  p \in border (nodes c) ->
  tree_like c ->
  partial_connected c ->
  weighted_count (fun c' => (tree_of_graph _ _ c' p == t) && tree_like c')
  [seq step_dist_it (lr k) c r (tval s) | s in dest_ports c #|border (nodes c)|]
  / weighted_count (@tree_like _)
  [seq step_dist_it (lr k) c r (tval s) | s in dest_ports c #|border (nodes c)|]
  = tree_dist lam rho t.
*)

Let build_graphs c r l :=
  [seq switch_step_dist_it lam rho c r (tval seqs)
  | seqs in dest_ports_seqs def_port c l.*2].

Lemma weighted_count_tree_of_trace l k c r p (t : tree l.*2 k) :
  p \in border (nodes c) ->
  partial_connected c ->
  weighted_count (fun c' => (tree_of_graph _ _ c' p == t) && tree_like c')
                 (build_graphs c r l) =
  \sum_(j in dest_ports_seqs def_port c l.*2 | (tree_of_trace k p j == t) &&
                    tree_like (switch_step_it c j))
   (switch_step_dist_it lam rho c r j).2.
Proof.
move=> Hpb Hpc.
rewrite /weighted_count big_map big_enum_in /=.
apply eq_bigl => /= j.
case Hj: (j \in _) => [/= | //].
rewrite switch_step_dist_it_1.
case Htl: (tree_like _); last by rewrite !andbF.
rewrite !andbT.
by rewrite (tree_of_trace_graph k Hpb Hj).
Qed.

Let dest_ports' c := dest_ports c #|border (nodes c)|.
Let tp (s : {set port}) (t : #|s|.-tuple (port * {set port})%type) :=
  tuple_to_partial (def_port, set0) t.

Lemma weighted_count_is_tree_dist l c r p (t : tree l.*2 kv) :
  r > 0 ->
  p \in border (nodes c) ->
  tree_like c ->
  partial_connected c ->
  weighted_count (fun c' => (tree_of_graph _ _ c' p == t) && tree_like c')
                 (build_graphs c r l)
  = tree_dist lam rho t * weighted_count (@tree_like _) (build_graphs c r l).
Proof.
elim: l c r p t => [|l IHl] c r p t Hr Hpb Htl Hpc.
  rewrite /build_graphs /= dest_ports_seqs_0.
  rewrite /weighted_count !big_map enum_set1 !big_cons !big_nil.
  by rewrite (tree_frontier t) eqxx /= Htl addr0 mul1r.
rewrite [in LHS]/build_graphs weighted_count_switch_step.
rewrite [in RHS](tree_node_children t) [RHS]/= /tree_dist_children.
rewrite big_map.
set f := fun c' : comp_graph port => _.
set g := \sum_(t0 in _) weighted_count _ _.
transitivity (\sum_(t1 in dest_ports' c)
    \sum_(t2 in dest_ports' (switch (step_it c t1)))
       \sum_(ts in dest_ports_seqs def_port
                   (switch (step_it (switch (step_it c t1)) t2)) l.*2 |
        (tree_of_trace kv p (cons_tuple (tp t1) (cons_tuple (tp t2) ts)) == t)
        && (tree_like
            (switch_step_it (switch (step_it (switch (step_it c t1)) t2)) ts)))
       (switch_step_dist_it lam rho c r (tp t1 :: tp t2 :: ts)).2).
  rewrite {}/g.
  apply eq_bigr => t1 Ht1.
  rewrite [LHS]/= tuple_to_partial_enumK.
  rewrite (surjective_pairing (step_dist_it _ _ _ _)) step_dist_it_1.
  rewrite weighted_count_switch_step.
  apply eq_bigr => t2 Ht2.
  rewrite /weighted_count big_map big_enum_in.
(* TODO(rei): script that fails with MathComp 1.6.1
  rewrite {}/f -doubleE.
  apply eq_big => ts.
    case Hts: (ts \in _); last done.
    rewrite [tree_like _]/=.
    rewrite [in tree_like _](surjective_pairing (step_dist_it _ _ _ _)).
    rewrite tuple_to_partial_enumK !step_dist_it_1 !switch_step_dist_it_1.
    case Htl': (tree_like _); last by rewrite !andbF.
    congr andb.
    congr andb.
    set ts' := cons_tuple _ _.
    rewrite (@tree_of_trace_graph def_port l.+1 kv c ts' p Hpb); first last.
    + done.
    + by rewrite /= !tuple_to_partial_enumK.
    + rewrite {}/ts' dest_ports_seqs_step.
      apply/bigcupP; exists t1 => //.
      rewrite mem_imset //.
      rewrite dest_ports_seqs_step.
      apply/bigcupP; exists t2 => //.
      by rewrite mem_imset.
    rewrite /ts'.
    set ss1 := switch_step_it _ _.
    set ss2 := switch_step_it _ _.
    suff : ss1 = ss2.
      by move=> ->.
    rewrite {}/ss1 {}/ss2 /=.
    by rewrite /tp !tuple_to_partial_enumK.
  move/and3P => [] Hts Htg Htl' /=.
  rewrite !tuple_to_partial_enumK.
  rewrite [in RHS](surjective_pairing (step_dist_it lam c r t1)) /tp.
  by rewrite step_dist_it_1 tuple_to_partial_enumK.
rewrite weighted_count_switch_step -mulrA big_distrr [RHS]/=.
move: (Hpb).
rewrite -mem_enum -index_mem -cardE => Hidx.
set px := Ordinal Hidx.
set card_en := fun t1 : #|border(nodes c)|.-tuple (port * {set port})%type =>
             Ordinal (leq_ltn_trans (max_card (pred_of_set (tnth t1 px).2))
                                    (ltnSn _)).
rewrite [in RHS](partition_big card_en predT) // [RHS]/=.
rewrite (bigID (fun t1 : #|border(nodes c)|.-tuple (port * {set port})%type =>
                 tree_like (step_it c t1) &&
                 (#|(tnth t1 px).2| == (size (tree_children t)).+1))).
rewrite [X in (_ + X) = _]big1; last first.
  move=> t1 /andP [Ht1 Hnt1].
  rewrite big1 // => t2 Ht2.
  rewrite big1 // => ts /and3P [Hts Htr].
  move/tree_like_rev_switch_step_it.
  rewrite tree_like_switch.
  move/tree_like_rev_step_it.
  rewrite tree_like_switch => Htl'.
  elimtype False.
  move: Htr.
  rewrite /= !tbeheadE !theadE (surjective_pairing (tp t1 p)).
  rewrite (tree_node_children t) => /eqP /Node_inj /(f_equal size).
  rewrite size_map -cardE => Hsz.
  move: Hnt1.
  rewrite Htl' -Hsz /=.
  move: (Ht1); rewrite inE => /andP [].
  have Hin: tp t1 p \in t1 by rewrite ffunE memt_nth.
  move/allP/(_ _ Hin).
  rewrite inE => /andP [_ Hp12] _.
  rewrite (cardsD1 (tp t1 p).1).
  move: Hp12.
  rewrite (tnth_nth (def_port, set0)) ffunE /= => ->.
  by rewrite eqxx.*)
Admitted.

Theorem weighted_count_is_fintree_dist l (t : fintree #|port| l.+1) :
  weighted_count (fun pnc => (fintree_of_graph l pnc == t) && tree_like pnc.2)
                 (next_graphs l)
  / weighted_count (@tree_like _ \o snd) (next_graphs l)
  = fintree_dist lam rho t.
Proof.
rewrite 2!weighted_count_next.
destruct t as [t Ht] => /=.
rewrite /fintree_dist /=.
rewrite [in RHS](tree_node_children t).
case/boolP: (tree_children t == [::]) => Hch /=.
  rewrite big1 ?mul0r // => pn Hpn.
  rewrite /weighted_count big_map.
  rewrite big1 // => /= i.
  rewrite /fintree_of_graph => /andP [] /eqP [].
  rewrite (tree_node_children t) (eqP Hch).
  rewrite /tree'_of_graph /image_mem => /Node_inj.
  move/nilP.
  rewrite /nilp size_map -cardE => /eqP Hc.
  by rewrite /start_dist Hc !eqxx in Hpn.
rewrite /fintree_of_graph /=.
Admitted.
...
Definition converges (e a b : K) := (b = 0 -> a = 0) \/ `|a / b - 1| < e.

Section correct_def.
Variables (port : finType) (h : hemi_comp_graph port).
Variable lam : NormalizedDegreeDistribution.L K.
(*Variable M : nat.*)
(*Hypothesis hM : (#|ports h| < M)%nat.*)
(* M = maxdeg * tree_max l.+1 *)
...
Proof. by rewrite ler_maxr lerr orTb. Qed.

Definition correct_dist0 :=
  \poly_(k < size lam)
      (remaining_ports k / \sum_(i < size lam) remaining_ports i).

Lemma correct_dist0_coef_ge0 i : 0 <= correct_dist0 `_ i.
Proof.
rewrite coef_poly; case: ifP => // _; rewrite divr_ge0 // ?rem_ports_ge0 //.
rewrite sumr_ge0 // => ? _; exact: rem_ports_ge0.
Qed.
...
(*Hypothesis : exists i : 'I_(size lam),
     Num.lt (i.+1 * #|[node x # i.+1 | x \in h]|)%:R (#|port|%:R * lam`_i)*)

(*
Let maxdeg := maxn (size lam) (size rho).
Let tree_max l : nat := \sum_(i < l) maxdeg.+1 ^ i.
Hypothesis Hmaxlen : forall i, lam`_i != 0 -> (maxdeg * tree_max l.+1)%:R < #|port|%:R * lam`_i.
*)

Lemma correct_dist0_1 : correct_dist0.[1] = 1.
Proof.
evar (f : 'I_(size lam) -> K).
rewrite horner_poly [LHS](_ : _ = \sum_i f i); last first.
...
Definition correct_dist : NormalizedDegreeDistribution.L K.
apply (NormalizedDegreeDistribution.mkL correct_dist0_coef_ge0).
exact: correct_dist0_1.
Defined.

End correct_def.

Lemma correct_dist_tends_to_dist (lam : NormalizedDegreeDistribution.L K) :
  forall e, exists E0 : nat, forall port : finType, (#|port| > E0)%nat ->
    forall k h,
      converges e ((@correct_dist port h lam) `_ k) (lam `_ k).
Proof.
move=> eps.
Admitted.

(* Should also prove it for the equivalent of switch_step_dist_it *)
Section step_dist.
Variable port : finType.

Definition step_dist' lam (c : comp_graph port) :=
  step_dist (correct_dist (conodes c) lam) c.

Definition step_dist_it' lam c r dests :=
  foldl
    (fun (cd : _ * K) (mapping : (port * (port * {set port}))%type) =>
       step_dist' lam cd.1 cd.2 mapping.1 mapping.2)
    (c,r) (zip (enum (border (nodes c))) dests).
...
Definition graph_of_trace' lam rho (s : {set port}) ts :=
  switch_step_dist_it' lam rho (start_graph s) (start_dist lam s) ts.
End step_dist.

Lemma graph_of_trace'_converges l lam rho :
  forall e, exists E0 : nat, forall E, (E > E0)%nat ->
    forall (s : {set 'I_E.+1}) (ts : l.-tuple _),
     converges e (graph_of_trace' lam rho s ts).2
                  (graph_of_trace lam rho (s,ts)).2.
Admitted.
End graph_dist.
...
Variable K : numFieldType.
Variable E : nat. (* number of ports *)

Section hemi_comp_graph.
Variables L : NormalizedDegreeDistribution.L K.

Record hemi_comp_graph (n (* number of nodes *): nat) := {
  part :> {set {set 'I_E}} ;
  part_p : partition part [set: 'I_E] ;
  card_p : #| part | = n ;
  deg_p : forall k, #| [set x in part | #| x | == k] | %:R = n%:R * L `_ k
}. (* NB: L : distribution of nodes by arity *)
End hemi_comp_graph.

Record comp_graph n m L R := {
  varnodes : hemi_comp_graph n L ;
  funnodes : hemi_comp_graph m R ;
  edges : {perm 'I_E}
}.

Record comp_graph_with_root n m L R (l : nat) := {
  fullgraph : comp_graph n m L R ;
  root : {set 'I_E} ;
  _ : root \in part (varnodes fullgraph)
}.
...
Variable K : numFieldType.
Variables E n m : nat.
Variables L R : NormalizedDegreeDistribution.L K.
Variable g : comp_graph E L R n m.

(* Should modify these definitions to extract a partial graph rather than
   a tree *)

Import TreeEnsemble.

(*
We need "Fixpoint extract_partial_graph"
that extracts a partial graph of depth n from a given complete graph.
The result should have the same distribution of shapes as the partial compgraphs
defined above
*)

Fixpoint extract_tree (l : nat) (prec : option 'I_E) (curr : {set 'I_E})
         (k : kind) : tree l k :=
  if l is l'.+1 then
    let nexts := if prec is Some p then curr :\ p else curr in
    let (edges, nodes) :=
      if k is kv then (edges g, part (funnodes g))
                 else ((edges g)^-1, part (varnodes g))%g in
    let nexts' := map edges (enum nexts) in
    let nodes :=
        [seq (p, [set x | [exists s in nodes, (x \in s) && (p \in s)]])
        | p <- nexts' ]
    in
    Node [seq extract_tree l' (Some pn.1) pn.2 (negk k) | pn <- nodes]
  else Frontier k.

Definition append_seqs (T U : finType) (s1 s2 : {set T} * {set U}) :=
  (s1.1 :|: s2.1, s1.2 :|: s2.2).

Fixpoint extract_nodes (l : nat) (prec : option 'I_E) (curr : {set 'I_E})
         (k : kind) : {set {set 'I_E}} * {set {set 'I_E}}  :=
  if l is l'.+1 then
    let nexts := if prec is Some p then curr :\ p else curr in
    let (edges, nodes) :=
      if k is kv then (edges g, part (funnodes g))
    ...
    in
    let pairs :=
        [seq extract_nodes l' (Some pn.1) pn.2 (negk k) | pn <- nodes] in
    let (s1, s2) := foldr (@append_seqs _ _) (set0, set0) pairs in
    (curr |: s2, s1)
  else (set0, set0).

Lemma extract_nodes_subset l prec curr k :
  let p := extract_nodes l prec curr k in
  let p := if k is kv then p else (p.2, p.1) in
  (p.1 \subset part (funnodes g)) && (p.2 \subset part (varnodes g)).
Admitted.

Variables (v : {set 'I_E}) (l : nat).
Let exnodes := extract_nodes l None v kv.

Lemma nodes_part_p : trivIset exnodes.1.
Admitted.
Lemma conodes_part_p : trivIset exnodes.2.
Admitted.

Let nodes_ports := \bigcup_(x in exnodes.1) x.
Let conodes_ports := \bigcup_(x in exnodes.2) x.
...
Definition partial_nodes :=
  PartialComputationGraph.Build_hemi_comp_graph nodes_part_p
         (subsetDl nodes_ports (edges g @^-1: conodes_ports)).

Definition partial_conodes :=
  PartialComputationGraph.Build_hemi_comp_graph conodes_part_p
         (subsetDl conodes_ports (edges g @: nodes_ports)).

Definition partial_graph :=
  @PartialComputationGraph.Build_comp_graph _  partial_nodes partial_conodes
      [ffun x => edges g x].
...
Definition check_siblings (r : option (seq 'F_2 * seq 'F_2)) (f : seq 'F_2 -> option ('F_2 * seq 'F_2)) :=
  if r is Some (bs, cw') then
    if f cw' is Some (b, cw'') then Some (b :: bs, cw'') else None
  else None.

Fixpoint check_codeword l k (t : tree l k) (cw : seq 'F_2) : option ('F_2 * seq 'F_2) :=
  match t, cw with
  | Frontier kv, b :: cw' => Some (b, cw')
  | Node _ kv s, b :: cw' =>
    let s' := map (@check_codeword _ _) s in
    let r := foldl check_siblings (Some ([::], cw')) s' in
    if r is Some (bs, cw'') then
      if all (pred1 b) bs then Some (b, cw'') else None
    else None
  | Node _ kf s, cw' =>
    let s' := map (@check_codeword _ _) s in
    let r := foldl check_siblings (Some ([::], cw')) s' in
    if r is Some (bs, cw'') then
      Some (\big[+%R/Zp0]_(a <- bs) a, cw'')
    else None
  | Frontier kf, _ => None
  | _, [::] => None
  end.

Definition EF2 := option 'F_2.

Definition decode_siblings (r : seq EF2 * seq EF2) (f : seq EF2 -> EF2 * seq EF2) :=
  let (bs, cw') := r in
  let (b, cw'') := f cw' in (b :: bs, cw'').

Definition product (b1 b2 : EF2) : EF2 :=
  if b1 is Some b then b1 else b2.

Definition sum (b1 b2 : EF2) :=
  match b1, b2 with
  | Some b, Some b' => Some (b + b')
  | _, _ => None
  end.

Fixpoint decode_word l k (t : tree l k) (cw : seq EF2) : EF2 * seq EF2 :=
  match t, cw with
  | Frontier kv, b :: cw' => (b, cw')
  | Node _ kv s, b :: cw' =>
    let s' := map (@decode_word _ _) s in
    let (bs, cw'') := foldl decode_siblings ([::], cw') s' in
    (foldl product b bs, cw'')
  | Node _ kf s, cw' =>
    let s' := map (@decode_word _ _) s in
    let (bs, cw'') := foldl decode_siblings ([::], cw') s' in
    if bs is b :: bs' then (foldl sum b bs', cw'') else (None, cw'')
  | Frontier kf, cw'' => (None, cw'')
  | _, [::] => (None, [::])
  end.

Variables (n l : nat) (t : fintree n l ).

Definition is_codeword (cw : seq 'F_2) := if check_codeword t cw is Some (_, [::]) then true else false.

(* NB: C^0(T) is when the first bit of the codeword is 0 *)

End ValidCodeword.

Require Import erasure_channel channel channel_code.
...
Variables eps : R.
Hypothesis eps01 : (0 <= eps <= 1)%R.
Let W : `Ch_1('F_2, [finType of option 'F_2]) := @EC.c [finType of 'F_2] _ eps01.
...
Definition c_dec : decT [finType of option 'F_2] [finType of 'rV['F_2]_n] n :=
  [ffun msg =>
  if decode_word t (tval (tuple_of_row msg)) is (Some x, _) then
    Some (\row_(i < n) if i == ord0 then x else Some (msg ord0 i)) else None].

Variable c : code [finType of 'F_2] [finType of option 'F_2] [finType of 'F_2] n.

Lemma theorem350 : e(W, c) = e(W, c).
Admitted.
