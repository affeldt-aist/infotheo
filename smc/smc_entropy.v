...
Variable pZ_unif : `p_ Z = fdist_uniform (card_ord p). (* Assumption in the paper. *)
...
rewrite -cinde_rv_unit.
rewrite /W2 /WmZ.
apply: lemma_3_5'. (* From WmZ to Wm. *)
  by exact pZ_unif.
rewrite -/W2.
apply: Z_WmW2_indep.
Qed.
...
Lemma pWmZ_unif:
  `p_ WmZ = fdist_uniform (card_ord p).
Proof.
rewrite /WmZ.
have H_ZWM := Z_Wm_indep.
rewrite inde_rv_sym in H_ZWM.
have H := add_RV_unif pZ_unif H_ZWM.
by exact H.
Qed.
...
Hypothesis card_Z : #|'rV[TX]_n| = m.+2. (* TODO: Makes sense? The number of instances of this type is at least two *)
